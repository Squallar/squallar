...
//! `rustdar` is the package whose executable the desktop rows upload.
//!
...
//! those — `rustdar-app`'s — carries a **dev**-dependency
//! `rustdar-radar = { features = ["test-support"] }`. Under resolver 2 that
//! feature is unified into the lib unit the binary links. Measured, by
//! counting `rustdar_radar` rustc invocations in `cargo check -v`:
//!
//! ```text
//! --workspace --all-targets --features rustdar/jni-typecheck  2 units, 2 with test-support
//! --workspace              --features rustdar/jni-typecheck  1 unit,  0 with test-support
//! -p rustdar --bins                                           1 unit,  0 with test-support
//! ```
//!
...
//! the iOS and macOS makefiles invoke `cargo … -p rustdar` with no feature
...
/// Workspace root: this crate's manifest dir is `rustdar/`.
#[cfg(test)]
...
mod tests {
    ...
    /// (`rustdar-web/tests/pwa_assets.rs` over `sw.js`,
