...
/// rustdar-location. An `EventLoopProxy::send_event` would surface as
...
static BACK_WAKER: std::sync::Mutex<Option<rustdar_app::platform::RedrawWaker>> =
...
#[cfg(target_os = "android")]
fn back_waker() -> std::sync::MutexGuard<'static, Option<rustdar_app::platform::RedrawWaker>> {
    BACK_WAKER
        .lock()
...
#[cfg(target_os = "android")]
pub(super) fn set_back_waker(waker: Option<rustdar_app::platform::RedrawWaker>) {
    *back_waker() = waker;
...
/// JClass for com.rustdar.BackHandler, loaded once via the app class loader.
