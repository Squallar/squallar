...
//! `squallar-source` walks the **transitive** closure to keep the
//! overlays→radar edge cut, so a first-party dependency added here would reach
//! `squallar-overlays` and could re-open that edge from underneath. Keeping this
...
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("squallar-netcdf sits one level under the workspace root")
        .to_path_buf()
}
...
fn the_crate_is_a_band_zero_leaf() {
    let meta = metadata();
    let deps = declared_deps(&meta, "squallar-netcdf");

    for (kind, name) in &deps {
        assert!(
            !name.starts_with("squallar-") && name != "nexrad-level3",
            "squallar-netcdf declares the first-party crate {name} ({kind}). \
             ...
             also lands in `squallar-overlays`, where the transitive charter in \
        ...
        assert!(
            allowed,
            "squallar-netcdf declares {name} ({kind}), which its charter does \
...
/// The extracted crate is actually reached: `squallar-overlays` stands on it.
///
...
fn the_format_layer_sits_under_the_data_crate() {
    ...
    assert!(
        overlays
            .iter()
            .any(|(kind, name)| kind == "normal" && name == "squallar-netcdf"),
        "squallar-overlays no longer stands on squallar-netcdf: {overlays:?}",
    );
}
