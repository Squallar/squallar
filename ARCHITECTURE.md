...
```bash
cargo test -p rustdar-egui parity_walk                    # 3/3, fourteen layers
cargo test -p rustdar-egui --features fake-source parity_walk   # 3/3, fifteen
```
...
and enabled by nothing that ships), is a fifteenth layer that exists only so a
test build can watch a source arrive through the seams. Every count-pin
downstream is written as `14 + cfg!(feature = "fake-source") as usize`, and
