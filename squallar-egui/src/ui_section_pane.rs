...
fn tilt_curves(
    ...
    let ranges: Vec<(f32, f64)> = (0..=TILT_CURVE_SAMPLES)
        .map(|i| {
            ...
            let (_, ground_km) = squallar_geo::site_bearing_range_km(site_lat, site_lon, lat, lon);
...
impl<'a> Ladder<'a> {
    ...
    /// [`squallar_radar::xsect::assembly_span_secs`], which is the one definition.
    fn span_secs(self) -> Option<i64> {
        squallar_radar::xsect::assembly_span_secs(self.collected_ms)
    }
...
fn describe_source(
    ...
    let (lat, lon) = squallar_geo::great_circle_point(a, b, t);
    let (_, ground_km) =
        squallar_geo::site_bearing_range_km(source.site_lat, source.site_lon, lat, lon);
