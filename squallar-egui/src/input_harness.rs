...
///
/// A [`ScanInfo`](squallar_radar::types::ScanInfo) is radar's fact about a scan
...
macro_rules! resolve {
    ($id:expr) => {
        squallar_radar::fields::product_for($id).expect("a registered field")
    };
}
...
struct FrameFactsForTest {
    ...
    overlay_overdraw: f32,
    location_settings_available: bool,
    location: (squallar_location::LocationPermission, bool),
    gps: Option<(squallar_location::Fix, web_time::Instant)>,
...
impl Default for FrameFactsForTest {
    fn default() -> Self {
        Self {
            ...
            location: (squallar_location::LocationPermission::default(), false),
            gps: None,
            user_heading: None,
...
fn install_radars() {
    ...
    ONCE.call_once(|| {
        ...
        squallar_radar::sites::resolve([("KCRI", SiteFix::Unplaced)]);
        ...
        squallar_radar::sites::resolve(PLACES.map(
...
impl InputHarness {
    ...
    fn apply_facts(&mut self) {
        ...
        self.gui.apply_frame_inputs(crate::shell_api::FrameInputs {
            ...
            tile_cache: self.facts.tile_cache,
            overlay_overdraw: self.facts.overlay_overdraw,
    ...
    pub(crate) fn set_location_state(
        &mut self,
        permission: squallar_location::LocationPermission,
        active: bool,
    ) {
    ...
    pub(crate) fn set_gps_fix(&mut self, fix: squallar_location::Fix) {
    ...
    pub(crate) fn control_item_model(
        &self,
        kind: &LayerId,
    ) -> Vec<squallar_overlays::render::controls::ControlItem> {
    ...
    pub(crate) fn pane_kinds(&self) -> Vec<squallar_radar::types::RenderView> {
        self.gui
            .panes()
            .iter()
    ...
    pub(crate) fn place_section(
        &mut self,
        idx: usize,
        axes: squallar_radar::xsect::SectionAxes,
        rungs: &[f64],
    ) {
        use squallar_radar::sampler::SampleStatus;
        use squallar_radar::xsect::{CrossSection, SECTION_HEIGHT, SECTION_WIDTH};
    ...
    pub(crate) fn load_scan(&mut self, site: &str) {
        let radar_site = squallar_radar::sites::get_radar_site(site).expect("unknown radar site");
        let info = squallar_radar::types::ScanInfo {
    ...
    pub(crate) fn offer_product(
        ...
        if !info.available_products.contains(&resolved) {
            ...
            info.available_products
                .sort_by_key(|p| squallar_radar::fields::spec(*p).sort_order);
        }
    ...
    pub(crate) fn place_radar_image(
        ...
        use squallar_radar::types::{BASE_EXTENT_KM, ImageBounds};
    ...
    pub(crate) fn place_site_at(&mut self, idx: usize, site: &str, target: egui::Pos2) {
        let radar = squallar_radar::sites::get_radar_site(site).expect("unknown radar site");
