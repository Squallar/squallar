...
/// **The pin under the pane-loop conversions in `squallar-app`'s `app_render.rs`.**
///
...
fn a_pane_kind_request_for_a_pane_that_is_gone_converts_nothing() {
    ...
    let mut gui = Gui::new();
    gui.request_pane_view(7, squallar_radar::types::RenderView::Volume);
    gui.apply_pending_pane_view(&mut Vec::new());
...
fn blank_section() -> squallar_radar::xsect::CrossSection {
    ...
    squallar_radar::xsect::CrossSection::from_parts(
        vec![0u8; pixels * 4],
        vec![f32::NAN; pixels],
...
fn overlay_polling_skips_panes_with_no_ground_but_keeps_their_toggles() {
    ...
    let kind = squallar_source::id::known::CITY_LABELS;
    let mut gui = Gui::new();
    gui.set_pane_count_for_test(2);
    for idx in 0..2 {
        gui.pane_mut(idx)
