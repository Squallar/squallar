...
use super::*;
use squallar_overlays::fetch_policy::{
...
const KIND: squallar_source::id::LayerId = squallar_source::id::known::SPC_DISCUSSIONS;

/// A `Gui` with exactly one auto-polling layer on screen, so the attempt count is
/// unambiguous.
...
/// The ladder, climbed through the real gate one rung at a time: 2 s doubling to
/// the layer's own poll interval, and never faster than that ceiling.
#[test]
...
/// The measured window, through the real gate: the 105 s that cost 3089 requests
/// now costs 6.
#[test]
...
/// A *run* of refusals is believed: the layer drops to the broken heartbeat rather
/// than the ceiling.
#[test]
...
/// **The recovery that did not recover.**
#[test]
...
fn a_discussion() -> squallar_overlays::spc::discussion::SpcDiscussion {
...
/// "Not published right now" is an answer, not a fault: it puts the layer back on
/// its ordinary interval instead of onto the ladder, and says so.
#[test]
...
fn a_dismissal_covers_one_failure_and_not_the_next() {
    const RADAR: squallar_source::id::LayerId = crate::radar_layer::POLL_LAYER;
    let mut gui = Gui::new();
