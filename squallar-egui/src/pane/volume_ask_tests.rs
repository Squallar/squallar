...
use squallar_source::handler::{FetchPayload, OverlayItem, RenderMode, Surface};
...
impl squallar_source::volume::VolumeCapable for StubLayer {
    ...
    /// exercised against the live handler in `squallar-app`, where a real
...
impl OverlayHandler for StubLayer {
    ...
    fn volume(&self) -> Option<&dyn squallar_source::volume::VolumeCapable> {
        self.capable
            .then_some(self as &dyn squallar_source::volume::VolumeCapable)
    }
}
