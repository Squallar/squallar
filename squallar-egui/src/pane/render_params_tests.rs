...
fn pane_listing(products: &[(FieldId, &[f32])]) -> PaneState {
    ...
    pane.scan_info = Some(ScanInfo {
        ...
        available_products: products
            .iter()
            .map(|(p, _)| squallar_radar::fields::product_for(p).expect("a registered field"))
            .collect(),
        product_elevations: products
            .iter()
            .map(|(p, angles)| {
                (
                    squallar_radar::fields::product_for(p).expect("a registered field"),
                    angles.to_vec(),
                )
            })
            .collect(),
...
fn the_data_time_on_screen_follows_the_loop_when_one_is_running() {
    ...
    *pane.time_state_mut(&known::RADAR) = crate::radar_layer::begin_loop(
    ...
    pane.time_state_mut(&known::RADAR).frames = vec![LoopFrame {
        timestamp: frame,
        image: None,
