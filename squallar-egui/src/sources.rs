...
mod retry_ledger_tests {
    ...
    /// The composed twin of `squallar-overlays`' own
    ...
    /// in `squallar-radar` and is chained in here. The overlays literal stays
...
mod controls_parity_tests {
    use squallar_overlays::render::controls::ControlItem;
    use squallar_overlays::render::overlay_state::{OverlayRegistry, PaneMut, PaneRef};
    use squallar_source::id::LayerId;
    ...
    fn the_radar_layer_still_offers_every_surface_that_moved_into_it() {
        use squallar_radar::source as radar;

        let registry = OverlayRegistry::with_handlers(all());
        let shapes: Vec<String> = registry
            .controls(&squallar_source::id::known::RADAR, &PaneRef::bare(0))
            .iter()
            .map(shape)
            .collect();
        for expected in [
...
mod registry_identity_tests {
    use squallar_source::id::LAYER_ID_LEDGER;
...
mod field_registry_tests {
    ...
    fn fields() -> Vec<(String, &'static squallar_source::product::ProductSpec)> {
        all()
            .iter()
            .flat_map(|h| {
