    ...
    pub(crate) fn drag_to(&mut self, pointer: egui::Pos2, ground: GeoPoint) {
        ...
        let next = match self.grab {
            ...
            SectionGrab::Body if self.sweep => {
                ...
                let (from_bearing, from_km) = squallar_geo::site_bearing_range_km(
                    mid.lat,
                    mid.lon,
                ...
                let (to_bearing, to_km) =
                    squallar_geo::site_bearing_range_km(mid.lat, mid.lon, ground.lat, ground.lon);
...
pub(crate) fn length_km(line: SectionLine) -> f64 {
    let (_, km) =
        squallar_geo::site_bearing_range_km(line.a().lat, line.a().lon, line.b().lat, line.b().lon);
    km
}

/// The line's midpoint on the ground — halfway along the great circle the cut
/// follows, from the same walk (`squallar_geo::great_circle_point`) the sampler and
...
pub(crate) fn bearing_deg(line: SectionLine) -> f64 {
    ...
    let (bearing, _) =
        squallar_geo::site_bearing_range_km(mid.lat, mid.lon, line.b().lat, line.b().lon);
    bearing
}
...
pub(crate) fn translated(line: SectionLine, from: GeoPoint, to: GeoPoint) -> Option<SectionLine> {
    let (motion_bearing, motion_km) =
        squallar_geo::site_bearing_range_km(from.lat, from.lon, to.lat, to.lon);
...
mod tests {
    ...
    fn pan_and_sweep_are_rigid_motions_of_the_line() {
        ...
        let (got_bearing, got_km) =
            squallar_geo::site_bearing_range_km(mid.lat, mid.lon, new_mid.lat, new_mid.lon);
        assert!(
        ...
        let (step_bearing, step_km) =
            squallar_geo::site_bearing_range_km(mid.lat, mid.lon, stepped_mid.lat, stepped_mid.lon);
        assert!(
    ...
    fn a_body_drag_translates_and_a_shift_body_drag_sweeps() {
        ...
        let (to_bearing, _) = squallar_geo::site_bearing_range_km(mid.lat, mid.lon, to.lat, to.lon);
        let (from_bearing, _) = squallar_geo::site_bearing_range_km(
            mid.lat,
            mid.lon,
            press_on_line.lat,
            press_on_line.lon,
