...
pub(super) struct SiteSections {
    ...
    pub distance: squallar_units::DistanceUnit,
}
...
fn site_row_label(name: &str, network: RadarNetwork, is_placed: bool) -> String {
    ...
    if let Some(place) = squallar_radar::sites::table().place(name) {
        label.push_str(" - ");
...
fn draw_site_sections(
    ...
    let mut draw = |ui: &mut egui::Ui,
        ...
        for (name, note) in entries {
            ...
            // places it.
            let placed = squallar_radar::sites::get_radar_site(&name).is_some();
...
pub(super) fn site_list_ui(
    ...
    let table = squallar_radar::sites::table();
...
fn shown_total_tdwrs(
    radars: &[squallar_radar::sites::RadarSite],
    unplaced: &[&'static str],
) -> usize {
    radars
        .iter()
...
impl super::Gui {
    ...
    pub(super) fn site_sections(&self, idx: PaneId, current: &str) -> SiteSections {
        ...
        let nearby = self.user_fix.as_ref().map_or_else(Vec::new, |fix| {
            squallar_radar::sites::nearest_sites(fix.point.lat, fix.point.lon, NEARBY_SITE_COUNT)
                .into_iter()
