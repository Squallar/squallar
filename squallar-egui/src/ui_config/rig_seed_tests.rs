...
//! `squallar-web`'s `pwa_assets` reads `worker_port.rs`: a copy of a literal is
...
///
/// `SEED_LS='{"squallar.ui": "…\"…\"…"}'` — single-quoted in the script, so the
...
fn seeded_config_json() -> String {
    ...
    outer
        .get("squallar.ui")
        .and_then(|v| v.as_str())
        .expect("the seed no longer carries a `squallar.ui` string")
        .to_string()
}
...
fn the_browser_rigs_scene_seed_enables_a_texture_overlay() {
    ...
    assert_eq!(
        ...
        Some(squallar_source::handler::RenderMode::Texture),
...
fn the_seeded_layer_is_one_a_fresh_pane_would_not_have() {
    ...
    let texture_layers: Vec<_> = gui
        .overlays
        .handlers()
        .filter(|h| h.render_mode() == squallar_source::handler::RenderMode::Texture)
        .map(|h| h.id())
        .filter(|id| *id != squallar_source::id::known::RADAR)
        .collect();
    let pane = gui.pane(0).expect("the seed asks for one pane");
