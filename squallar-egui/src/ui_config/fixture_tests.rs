...
fn a_future_builds_config_survives_a_session_with_every_unknown_intact() {
    ...
    let mystery = squallar_source::id::LayerId::new("FutureSatellite");
    assert_eq!(
...
fn a_config_naming_an_unregistered_layer_is_written_back_byte_preserved() {
    ...
    let retired = squallar_source::id::LayerId::new(RETIRED_ID);
    assert!(
...
fn a_corrupt_top_level_field_resets_to_its_default_and_the_rest_loads() {
    ...
    assert_eq!(
        ...
        serde_json::to_value(squallar_units::UserPreferences::default()).expect("serializable"),
...
fn a_v1_gps_config_splits_into_serial_config_and_a_root_heading() {
    ...
    assert_eq!(
        gui.heading_source,
        squallar_location::HeadingSource::CompassOnly,
    );
...
fn an_unknown_draw_order_id_survives_in_place_and_is_skipped_at_draw() {
    ...
    let mystery = squallar_source::id::LayerId::new("MysteryLayer");
    ...
    let pos = |id: &squallar_source::id::LayerId| {
        order
            .iter()
            .position(|k| k == id)
    ...
    let joined_or_saved = |id: &squallar_source::id::LayerId| order.contains(id);
    for id in [
        known::NWS_ALERTS,
...
fn the_radar_handler_and_the_pane_do_not_claim_the_same_slot_members() {
    ...
    let handler = squallar_radar::source::RadarSource::new();
    let fresh = handler
...
fn two_panes_hold_different_answers_for_one_layer_and_both_survive_a_reopen() {
    ...
    // apart, so it stays.
    gui.overlays
        .set_enabled(&kind, true, &mut squallar_source::handler::PaneMut::bare(0));
...
fn two_panes_hold_different_hrrr_parameters_through_every_read_and_a_reopen() {
    use squallar_overlays::hrrr::{GridCoords, HrrrFetchResult, HrrrGridData, ModelParameter};
    use squallar_overlays::render::controls::{ControlItem, ControlUpdate, ControlValue};
    use squallar_overlays::render::overlay_state::{OverlayFetchResult, RasterizeContext};
...
fn a_pre_field_id_config_keeps_every_volume_curve_on_its_own_product_and_its_own_spelling() {
    ...
    assert!(
        squallar_radar::fields::product_for(&unknown).is_none(),
        "premise"
    );
