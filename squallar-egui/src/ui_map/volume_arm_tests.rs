...
fn an_unregistered_field_is_refused_by_name_rather_than_resolved_to_the_default_row() {
    ...
    assert!(
        squallar_radar::fields::spec_for(&alien).is_none(),
...
fn a_derived_product_is_asked_for_rather_than_refused_by_name() {
    for product in [
        ...
        assert!(
            squallar_radar::fields::product_for(&product)
                .is_none_or(|p| squallar_radar::sampler::samplable(p).is_none()),
...
fn converting_a_volume_pane_away_releases_its_volume() {
    let (mut h, _painter) = volume_harness(StubVolumePainter::painting());
    h.gui_mut()
        .request_pane_view(1, squallar_radar::types::RenderView::PlanView);
    h.frames_for(1, FRAME_DT);
...
fn converting_a_map_pane_releases_nothing() {
    let (mut h, _painter) = volume_harness(StubVolumePainter::painting());
    h.gui_mut()
        .request_pane_view(0, squallar_radar::types::RenderView::CrossSection);
    h.frames_for(1, FRAME_DT);
    assert!(
        !h.last_actions()
            .iter()
...
const GUARANTEE_CELLS: usize =
    squallar_radar::voxel::shape_for_budget(squallar_radar::voxel::DESKTOP_SHAPE, 256).nx;
...
fn the_caption_reports_the_resolution_the_box_buys() {
    let whole_volume = squallar_radar::voxel::box_half_width_km(460.125);
...
fn the_3d_ask_names_the_layer_that_builds_it() {
    let (mut h, _painter) = volume_harness(StubVolumePainter::painting());
    h.frames_for(1, FRAME_DT);
    let asked: Vec<squallar_source::id::LayerId> = h
        .last_actions()
        .iter()
        .filter_map(|a| match a {
