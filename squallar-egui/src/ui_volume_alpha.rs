...
fn absent_curve_message(facts: &squallar_source::product::ProductSpec) -> String {
    if !facts.vertical {
        format!(
...
fn editor_contents(
    ui: &mut egui::Ui,
    pane_idx: usize,
    facts: &squallar_source::product::ProductSpec,
    painter: Option<&dyn crate::volume_view::VolumePainter>,
    target: Option<&crate::pane::VolumeTarget>,
    curves: &mut AlphaCurves,
) {
        ...
        for id in [
            ...
            assert!(
                squallar_radar::sampler::samplable(
                    squallar_radar::fields::product_for(&facts.id).expect("registered")
                )
                .is_none(),
