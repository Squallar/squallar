...
impl RailRegions {
    ...
    /// the layer's own [`squallar_source::time::TimeAxis`] step, anchored on
...
impl super::Gui {
    ...
    fn rail_regions(&self, pane_idx: usize) -> RailRegions {
        ...
        let squallar_source::time::TimeAxis::FrameSeries {
            typical_step,
            extends_future: true,
        } = handler.time_axis()
        else {
    ...
    fn tuning_scope_caption(&self, pane_idx: usize) -> String {
        ...
        let step = match handler.time_axis() {
            squallar_source::time::TimeAxis::FrameSeries { typical_step, .. } => {
