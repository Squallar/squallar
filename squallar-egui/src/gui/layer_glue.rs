...
impl Gui {
    ...
    pub(crate) fn dropdown_model_for_test(
        ...
        let registered: Vec<squallar_source::id::LayerId> =
    ...
    ///
    /// The table lives in `squallar-radar`, which `squallar-overlays` must not
    ...
    pub fn publish_radar_sites(&mut self) {
        ...
        self.published_sites_generation = squallar_radar::sites::table_generation();
        let rows: Vec<SiteRow> = squallar_radar::sites::radars()
            .iter()
            .map(|site| SiteRow {
    ...
    /// [`squallar_radar::sites::radars`]. Two readers of one table, and only
    ...
    pub fn republish_radar_sites_if_the_table_moved(&mut self) -> bool {
        if self.published_sites_generation == squallar_radar::sites::table_generation() {
            return false;
        }
    ...
    /// table is named
    /// [`UNKNOWN_SITE_NAME`](squallar_radar::sites::UNKNOWN_SITE_NAME), and
    ...
    pub fn deliver_overlay_fetch(
        &mut self,
        result: squallar_overlays::render::overlay_state::OverlayFetchResult,
    ) {
    ...
    ///
    /// This is the only door `squallar-egui` and the shell have to a handler's
    ...
    pub(super) fn layer_error(&self, id: &LayerId) -> Option<String> {
        ...
        let message = match retry.health() {
            squallar_overlays::fetch_policy::FetchHealth::Failing { message, .. }
            | squallar_overlays::fetch_policy::FetchHealth::Broken { message } => message.clone(),
            // Neither is a failure the user is asked to acknowledge.
            squallar_overlays::fetch_policy::FetchHealth::Ok
            | squallar_overlays::fetch_policy::FetchHealth::Absent => return None,
        };
        ...
        self.across_panes(&id, |overlays, pane| {
            match outcome {
                ...
                RoundOutcome::Failed(message) => overlays.record_fetch_failure(
                    &id,
                    &squallar_source::fetch_policy::FetchError::transient(message),
                    pane,
                ),
            }
        });
