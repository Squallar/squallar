...
fn workspace_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("squallar-geo sits one level under the workspace root")
        .to_path_buf()
}
...
fn the_dependency_ceiling_holds() {
    let meta = metadata();
    let deps = declared_deps(&meta, "squallar-geo");
    ...
    for (kind, name) in &deps {
        let allowed: bool = match kind.as_str() {
            ...
            "normal" => false,
            other => panic!(
                "squallar-geo declares a `{other}` dependency on {name}; \
                 the charter permits none"
            ),
        };
        assert!(
            allowed,
            "squallar-geo declares {name} ({kind}). squallar-geo is the \
    ...
    assert!(
        ...
        "squallar-geo no longer declares serde_json (dev) — either the crate \
         changed shape or this test is reading the wrong package: {deps:?}",
    );
}

/// squallar-source stands on squallar-geo, so every crate above the substrate
...
fn the_floor_sits_under_the_substrate() {
    let meta = metadata();
    let source = declared_deps(&meta, "squallar-source");

    assert!(
        source
            .iter()
            .any(|(kind, name)| kind == "normal" && name == "squallar-geo"),
        "squallar-source no longer stands on squallar-geo: {source:?}",
    );
}
