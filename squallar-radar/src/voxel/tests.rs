...
fn the_centre_may_sit_away_from_the_site() {
    ...
    let east_lon = SITE.1 + 50.0 / (squallar_geo::KM_PER_DEGREE_LAT * SITE.0.to_radians().cos());
    let req = VoxelRequest {
...
fn serial_reference_grid(
    ...
    let (bearing_deg, range_km) =
        squallar_geo::site_bearing_range_km(lat, lon, req.centre.0, req.centre.1);
...
fn an_unregistered_field_shapes_no_request() {
    let alien = squallar_source::product::FieldId::from_static("NotAFieldThisBuildHas");
    assert!(
