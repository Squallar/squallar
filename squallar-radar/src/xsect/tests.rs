...
fn point_at(bearing_deg: f64, range_km: f64) -> (f64, f64) {
    let ang = range_km / squallar_geo::EARTH_RADIUS_KM;
...
fn every_pixel_is_the_volume_sampled_at_that_pixels_own_place() {
    ...
    for &col in &cols {
        ...
        let point = squallar_geo::great_circle_point(req.start, req.end, t);
        let (azimuth, ground) =
            squallar_geo::site_bearing_range_km(SITE.0, SITE.1, point.0, point.1);
        for &row in &rows {
...
fn the_ground_track_and_the_range_ring_are_the_same_sphere() {
    let ring_deg = types::BASE_EXTENT_KM / squallar_geo::KM_PER_DEGREE_LAT;

    // The same latitude offset, measured the way this module measures it.
    let ring_point = (SITE.0 + ring_deg, SITE.1);
    let (_, ours_km) =
        squallar_geo::site_bearing_range_km(SITE.0, SITE.1, ring_point.0, ring_point.1);
    ...
    assert_eq!(
        ...
        (squallar_geo::EARTH_RADIUS_KM * std::f64::consts::PI / 180.0).to_bits(),
...
fn a_line_across_the_site_blinds_one_column_and_flips_the_bearing() {
    ...
    let ranges: Vec<f64> = (0..SECTION_WIDTH)
        .map(|col| {
            ...
            let p = squallar_geo::great_circle_point(req.start, req.end, t);
            squallar_geo::site_bearing_range_km(SITE.0, SITE.1, p.0, p.1).1
        })
        .collect();
    let min_col = ranges
        .iter()
    ...
    let bearing = |col: usize| {
        let t = axes.column_distance_km(col) / axes.length_km;
        let p = squallar_geo::great_circle_point(req.start, req.end, t);
        squallar_geo::site_bearing_range_km(SITE.0, SITE.1, p.0, p.1).0
    };
