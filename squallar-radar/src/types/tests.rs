...
fn a_corrected_position_reaches_every_consumer_of_it() {
    ...
    let (was_bearing, was_range) =
        squallar_geo::site_bearing_range_km(table.lat, table.lon, target.0, target.1);
    let (now_bearing, now_range) =
        squallar_geo::site_bearing_range_km(moved.0, moved.1, target.0, target.1);
...
/// [`squallar_geo::mercator_y_from_sin_lat`] is [`squallar_geo::lat_rad_to_mercator_y`],
...
fn the_mercator_y_from_a_sine_is_the_one_from_an_angle() {
    ...
    for i in -8_900..=8_900 {
        ...
        let from_angle = squallar_geo::lat_rad_to_mercator_y(lat_rad);
        let from_sine = squallar_geo::mercator_y_from_sin_lat(lat_rad.sin());
