...
pub use squallar_source::volume::{
...
pub fn horizontal_ranges_km(
    ...
    let (bearing_deg, range_km) =
        squallar_geo::site_bearing_range_km(site_lat, site_lon, centre.0, centre.1);
...
// `squallar-worker`'s `offload`. The frame is self-delimiting and
