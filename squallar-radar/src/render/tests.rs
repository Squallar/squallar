...
fn probe_at(extent_km: f64, side_px: usize, az_deg: f64, range_km: f64) -> usize {
    ...
    let (sin_d, cos_d) = (range_km / squallar_geo::EARTH_RADIUS_KM).sin_cos();
...
fn painted_ranges_km_at(values: &[f32], extent_km: f64, site_lat: f64) -> Vec<f64> {
    ...
    (0..side)
        ...
        .map(|(row, col)| {
            ...
            squallar_geo::site_bearing_range_km(site_lat, LON, lat, lon).1
        })
        .collect()
}
...
fn a_tdwr_doppler_sweep_is_projected_at_its_own_reach_not_the_base_extent() {
    ...
    for (name, lat, lon) in [
        ...
        let ring_km = (bounds.max_lat - lat) * squallar_geo::KM_PER_DEGREE_LAT;
        assert!(
        ...
        for extent_km in [88.8, 150.0, 230.0, 416.98, 460.11] {
            ...
            for i in 0..720 {
                ...
                for frac in [0.01, 0.25, 0.5, 0.75, 1.0] {
                    ...
                    let (lat, lon) =
                        squallar_geo::great_circle_destination(site_lat, LON, az, range_km);
                    let want_px =
                        (lon - bounds.min_lon) / (bounds.max_lon - bounds.min_lon) * side as f64;
                    let merc_y = squallar_geo::lat_rad_to_mercator_y(lat.to_radians());
