...
const ALLOWED: &[(&str, &str, &str)] = &[
    ...
    (
        "squallar-radar/src/beam/tests.rs",
...
fn horizontal_geodesy_has_exactly_one_definition() {
    ...
    assert!(
        ...
        "{} site(s) name an earth radius, a kilometres-per-degree figure or \
         ...
         back — use `squallar_geo::KM_PER_DEGREE_LAT` (or \
         ...
         ends at, use `squallar_geo::MERCATOR_LAT_LIMIT_DEG`: `85.05` \
...
fn only_files_named_as_tests_are_exempt() {
    for path in [
        "squallar-egui/src/tiles/tests.rs",
        "squallar-egui/src/overlay_cache/texture_budget_tests.rs",
        "squallar-radar/tests/geodesy_one_definition.rs",
    ] {
        assert!(is_test_file(path), "`{path}` is a test file");
    }
    for path in [
        "squallar-overlays/src/render/rasterize.rs",
        "squallar-egui/src/tiles.rs",
        "squallar-radar/src/types.rs",
        // Not a test file: `tests` is a path component of neither, and the
        // exemption must not spread by substring.
        "squallar-radar/src/latest.rs",
...
fn the_exemption_does_not_reach_the_geodesy_bands() {
    ...
    assert!(
        hits.iter()
            .any(|h| h.path.ends_with("squallar-radar/src/beam/tests.rs")),
