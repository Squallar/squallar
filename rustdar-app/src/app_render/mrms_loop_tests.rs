...
fn build_loop(app: &mut crate::app::App) {
    ...
    *pane.time_state_mut(&known::MRMS) = rustdar_egui::pane::LayerTimeState::begin(
    ...
    let keys: Vec<(chrono::NaiveDateTime, String)> = (0..FRAMES)
        .map(|k| {
            (
                t(k),
                rustdar_source::origins::DataSources::mrms_key(PRODUCT.prefix_name(), &t(k)),
            )
        })
        .collect();
    app.channels
...
///    can be injected — see `rustdar-overlays`);
...
/// `rustdar-egui`'s `radar_takes_the_clock_wherever_it_is_drawn` goes red
/// with it.
#[test]
