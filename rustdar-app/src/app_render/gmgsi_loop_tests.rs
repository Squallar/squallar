...
fn a_frame_owed_its_granule_is_asked_for_once_however_many_passes_run() {
    let ctx = egui::Context::default();
    let _guard = rustdar_worker::offload::install_test_worker(Box::new(DescribedJobs::default()));
    let mut app = satellite_app();
    build_loop(&mut app);
...
///
/// `cargo test -p rustdar-app --lib -- --ignored --nocapture live_gmgsi`
#[test]
...
fn live_a_satellite_pane_plays_its_loop_end_to_end() {
    ...
    for step in 0..1800usize {
        ...
        if matches!(ls.phase, rustdar_egui::pane::LoopPhase::Playing) && pictures == ls.frames.len()
        {
