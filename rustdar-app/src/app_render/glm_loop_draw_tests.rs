...
//!
//! `rustdar-overlays`' `a_poll_landing_on_the_loops_oldest_frame_still_fills_the_whole_sweep`
...
//! `rustdar-overlays`. What this suite adds is the half no fetch test can see:
...
#[derive(Default)]
struct Jobs {
    seen: Arc<Mutex<Vec<rustdar_source::job::DescribedJob>>>,
}

impl rustdar_worker::offload::JobSink for Jobs {
    fn send(
        &self,
        _id: u64,
...
fn plan() -> rustdar_egui::overlay_cache::OverlayTexturePlan {
    rustdar_egui::overlay_cache::OverlayTexturePlan {
        width: 64,
        height: 64,
        overdraw: 0.0,
...
fn poll_lightning(app: &mut crate::app::App) -> usize {
    ...
    app.channels
        ...
        .send(SourceEvent::Data(
            rustdar_source::handler::OverlayFetchResult {
...
fn strikes_in(job: &rustdar_source::job::DescribedJob) -> Option<Vec<i64>> {
    let input = job.downcast_ref::<rustdar_overlays::render::rasterize::GlmStrikesInput>()?;
    Some(
        (0..FRAMES)
            .filter(|&k| {
                let out = rustdar_overlays::render::rasterize::rasterize_glm_strikes(
                    input,
                    &probe(k),
                    64,
                    64,
...
fn overlay_pass(
    ...
    let token = {
        ...
        rustdar_egui::overlay_cache_token(&app.gui.overlays, 0, pane, &known::LIGHTNING, false)
    };
    let stale = {
...
fn every_frame_of_a_satellite_loop_draws_its_own_lightning() {
    ...
    let _guard = rustdar_worker::offload::install_test_worker(Box::new(jobs));
    ...
    // already on a loop clock when lightning is asked for.
    {
        ...
        pane.set_time_mode(rustdar_egui::pane::TimeMode::AsOf(hour(0)));
    }
