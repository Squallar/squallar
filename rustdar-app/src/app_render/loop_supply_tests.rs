...
impl OverlayHandler for SupplyLayer {
    ...
    fn create_frame_list_task(
        ...
        let frames: Vec<FrameStamp> = self
            ...
            .collect();
        Some(rustdar_source::handler::FrameListingResult::task(
            self.id.clone(),
            async move {
                rustdar_source::handler::FrameListingResult {
...
fn awaiting_listing(
    ...
    *pane.time_state_mut(&supply_id()) = rustdar_egui::pane::LayerTimeState::begin(
...
fn the_residency_oracle_answers_one_frame_at_a_time() {
    ...
    let mut ls = rustdar_egui::pane::LayerTimeState::new();
...
fn the_radar_arm_produces_the_same_loop_it_always_did() {
            ...
            scope: Box::new(rustdar_radar::source::RadarListing {
                ...
                scans: scans
                    ...
                    .map(|ts| {
                        (
                            *ts,
                            rustdar_radar::archive::Identifier::new(format!("KTLX{ts}")),
                        )
                    })
                    .collect(),
            }),
    ...
    assert_eq!(ls.phase, rustdar_egui::pane::LoopPhase::Rendering);
    assert_eq!(
        ls.current_frame(),
        ls.frames.len() - 1,
