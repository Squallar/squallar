...
fn one_frame(app: &mut crate::app::App, ctx: &egui::Context, time: f64) -> Vec<LayerId> {
    ...
    let asked = actions
        ...
        .filter_map(|a| match a {
            rustdar_egui::actions::GuiAction::RenderOverlay { overlay_kind, .. } => {
...
fn an_idle_pane_asks_for_no_further_rasters_once_its_layers_hold_a_picture() {
    ...
    // `rustdar_radar::sites::table_generation` moves — and that table is a
    ...
    // `rustdar-app` suite running in parallel this fires, and with this test
    ...
    let table_before = rustdar_radar::sites::table_generation();
    ...
    let table_moved = rustdar_radar::sites::table_generation() != table_before;
