...
fn active_loop(stamps: &[chrono::NaiveDateTime]) -> LayerTimeState {
    let mut ls = rustdar_egui::radar_layer::begin_loop(3600, &site(), RenderView::PlanView);
...
fn textured(ctx: &egui::Context) -> rustdar_egui::pane::LoopFrameImage {
    let image = egui::ColorImage::from_rgba_unmultiplied([1, 1], &[255, 255, 255, 255]);
    rustdar_egui::pane::LoopFrameImage::PlanView(rustdar_egui::pane::RadarImageData {
        texture: ctx.load_texture("donor", image, egui::TextureOptions::NEAREST),
        lat: 35.33,
        lon: -97.27,
        max_range_km: 230.0,
        placed: rustdar_radar::types::ImageBounds::from_radar_site(35.33, -97.27, 230.0).into(),
        nyquist_ms: None,
...
fn radars_frames_after_a_listing(park_on_the_satellite: bool) -> Vec<chrono::NaiveDateTime> {
    ...
    let mut app = n_pane_app(1, SITE);
    {
        ...
        *pane.time_state_mut(&known::RADAR) =
            rustdar_egui::radar_layer::begin_loop(600, &site(), RenderView::PlanView);
    ...
    let scans: Vec<(chrono::NaiveDateTime, rustdar_radar::archive::Identifier)> = stamps
        .iter()
        .map(|&valid| {
            (
                valid,
                rustdar_radar::archive::Identifier::new(format!("{SITE}{valid}")),
            )
        })
        .collect();
    app.channels
...
fn frames_holding_a_picture_after_a_render(park_on_the_satellite: bool) -> (bool, bool) {
    ...
    app.loop_mgr = rustdar_radar::loop_downloads::LoopDownloadManager::new();
