...
const OVERSIZED: usize = rustdar_device_profile::constants::LONG_RANGE_IMAGE_SIZE;
...
fn the_restore_runs_from_inside_the_frame_and_not_from_the_state_that_built_it() {
    ...
    // more App-pokes-Gui occurrence in `rustdar-app`, and it would spend a
...
fn a_cross_section_can_never_be_the_raster_that_does_not_fit() {
    ...
    let section = rustdar_radar::xsect::SECTION_WIDTH.max(rustdar_radar::xsect::SECTION_HEIGHT);
    assert!(
        section <= floor,
