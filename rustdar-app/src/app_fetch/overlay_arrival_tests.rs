...
impl rustdar_worker::offload::JobSink for CountingPort {
    fn send(
        &self,
        _id: u64,
...
fn an_alert_round(n: usize) -> rustdar_overlays::render::overlay_state::FetchPayload {
    let alerts = (0..n)
...
fn a_reports_round(n: usize) -> rustdar_overlays::render::overlay_state::FetchPayload {
...
fn draw_loop_token(app: &mut crate::app::App, idx: usize, id: &LayerId) -> u64 {
    ...
    rustdar_egui::overlay_cache_token(overlays, idx, &panes[idx], id, is_dark)
}
...
fn seeded(
    id: &LayerId,
    round: rustdar_overlays::render::overlay_state::FetchPayload,
) -> crate::app::App {
...
fn an_arrival_that_does_not_move_the_token_dispatches_nothing() {
    ...
    let _guard = rustdar_worker::offload::install_test_worker(Box::new(CountingPort {
...
fn both_paths_write_the_record_and_reach_the_dispatch_through_one_entry() {
    ...
    app.process_gui_actions(vec![rustdar_egui::actions::GuiAction::RenderOverlay {
        pane_idx: 0,
...
fn the_arrival_collection_names_no_dispatch_of_its_own() {
    ...
    const UI_MAP_PANE: &str = include_str!("../../../rustdar-egui/src/ui_map_pane.rs");
    assert_eq!(
    ...
    assert!(
        APP_FETCH.contains("rustdar_egui::overlay_cache_token("),
