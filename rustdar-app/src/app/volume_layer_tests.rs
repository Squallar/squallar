...
fn target(product: rustdar_source::product::FieldId) -> rustdar_egui::pane::VolumeTarget {
...
fn an_ask_naming_a_layer_with_no_3d_half_is_refused_by_name() {
    ...
    let flat = rustdar_source::id::known::NWS_ALERTS;
    let why = refusal(
        &mut app,
        &flat,
        &target(rustdar_radar::fields::known::REFLECTIVITY),
    )
...
fn a_refused_ask_stops_the_level_trigger_re_asking() {
    ...
    let t = target(rustdar_radar::fields::known::REFLECTIVITY);
    app.handle_prepare_volume(0, &rustdar_source::id::known::NWS_ALERTS, t.clone());
    assert_eq!(
        app.gui
            .pane(0)
            .expect("pane 0")
...
fn an_ask_naming_a_layer_this_build_does_not_serve_is_refused_by_id() {
    let mut app = headless(TestBridge::desktop());
    let unknown = rustdar_source::id::LayerId::new("a.layer.from.a.newer.build");
    let why = refusal(
        &mut app,
        &unknown,
        &target(rustdar_radar::fields::known::REFLECTIVITY),
...
fn an_ask_for_a_field_with_no_vertical_structure_is_refused_by_name() {
    let mut app = headless(TestBridge::desktop());
    let flat_field = rustdar_radar::fields::known::ECHO_TOPS;
    let spec = rustdar_radar::fields::spec_for(&flat_field).expect("a registered field");
    assert!(
        !spec.vertical,
...
fn an_ask_for_a_holder_beyond_the_pane_vector_is_still_resolved() {
    ...
    let t = target(rustdar_radar::fields::known::REFLECTIVITY);
    app.handle_prepare_volume(beyond, &rustdar_source::id::known::NWS_ALERTS, t.clone());
    assert!(
        matches!(
