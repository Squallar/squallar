...
squallar a launcher entry, an icon, a window that groups under that entry, and
...
squallar used to talk to [GeoClue2] directly. GeoClue refuses
...
squallar now uses **`org.freedesktop.portal.Location`** and nothing else. The
...
squallar reports that as **Denied** — a decision, reversible, not a missing
...
backend implementing `Location` — squallar reports **Unavailable** instead, and
...
```sh
cargo build --release -p squallar --bin squallar
...
|---|---|---|
| file name | `app.squallar.desktop` | The basename is the application id. It matches `ios/project.yml`'s bundle id and the Android `applicationId`, and a test in `os_location/linux.rs` compiles this file in by that exact path, so renaming it fails the build. |
| `Icon=` | `app.squallar` | A bare identifier, not a path. The icon theme spec resolves it against the installed `hicolor` sizes; a path would pin one size and defeat the lookup. |
| `StartupWMClass=` | `squallar` | See below. |
| `Exec=` | `squallar` | The binary this repo builds. Rename it and `StartupWMClass` has to change with it. |
...
squallar does not do, so **a native Wayland window carries no `app_id` at all**.
The consequence is worth knowing before filing a bug about it:

* Under X11 or XWayland the window's `WM_CLASS` is `squallar`, which is
...
`WindowAttributesExtWayland::with_name` in `squallar-app`; it is out of
scope here and is noted rather than papered over.

## Icons

There is no `icons/` directory here on purpose. The Makefile installs
`squallar-web/icons/{favicon-32,icon-192,icon-512}.png` — the repo's only raster
art — into `hicolor/{32x32,192x192,512x512}/apps/app.squallar.png`. A
...
squallar neither makes nor can suppress that request — it is `geoclue.conf`'s
...
ls ~/.local/share/applications/app.squallar.desktop
gio info ~/.local/share/applications/app.squallar.desktop   # optional
```

Then run the app with `RUST_LOG=squallar_native=debug` and press "Use my
...
## One failure mode that looks like a squallar bug and is not

If the portal answers with response code 2 — squallar logs "the portal accepted
...
interaction; squallar has no part in it beyond reporting the failure honestly and
