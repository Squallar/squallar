 ...
 * edge and nothing else. The source pin in rustdar-app/src/app/tests.rs greps
 ...
 * register_java_helper in rustdar/src/android/mod.rs.
 */
object BackHandler {

    private const val TAG = "rustdar"

    init {
        // MEASURED, not defensive (WO-RP-SPIKE, 2026-08-20): without this the
        // callback below reaches `nativeBackPressed()` and ART answers
        //
        //   No implementation found for void com.rustdar.BackHandler.nativeBackPressed()
        //   (tried Java_com_rustdar_BackHandler_nativeBackPressed ...)
        //   - is the library loaded, e.g. System.loadLibrary?
        //
        // even though the symbol is exported from librustdar_native.so and the
        ...
        // cdylib to re-run. The name is rustdar's [lib] name, which is also the
        ...
        System.loadLibrary("rustdar_native")
    }
     ...
     * has no decision left to express. Implemented in librustdar_native.so as
     * `Java_com_rustdar_BackHandler_nativeBackPressed`
     * (rustdar/src/android/back.rs).
     *
