...
pub struct CachedPaneRender {
    ...
    /// [`squallar_radar::hover::HoverSource`].
...
pub struct RenderDispatcher {
    ...
    /// `squallar_radar::srv::storm_motion`.
    ...
    last_srv_fallback: squallar_radar::srv::SrvFallback,
    ...
    extract_cache: HashMap<ExtractKey, Arc<squallar_radar::render_input::RenderInput>>,
...
/// arguments [`squallar_radar::render_input::RenderInput::extract`] takes**
...
pub(crate) type ExtractResult = (ExtractKey, Arc<squallar_radar::render_input::RenderInput>);
...
pub struct MeltingLayerObject {
    ...
    /// `squallar_radar::level3::fetch_product_for_volume`.
...
struct SectionInput {
    ...
    /// owned `RenderInput`.
    input: Arc<squallar_radar::render_input::RenderInput>,
}
...
impl SectionInputKey {
    ...
    fn of(
        target: &squallar_egui::pane::SectionTarget,
        motion: Option<(f32, f32)>,
        fallback: squallar_radar::srv::SrvFallback,
    ) -> Self {
        Self {
...
fn plan_view_image(rgba: &[u8]) -> Option<egui::ColorImage> {
    let Some(side) = squallar_device_profile::constants::raster_side_from_rgba_len(rgba.len())
    else {
        log::error!(
...
impl RenderDispatcher {
    ...
    pub fn new() -> Self {
        Self::with_budgets(&squallar_device_profile::budget::resolve(
    ...
    pub fn with_budgets(budgets: &Budgets) -> Self {
        ...
        Self {
            ...
            last_srv_fallback: squallar_radar::srv::SrvFallback::default(),
            section_input: None,
    ...
    pub fn set_storm_motion_choice(
        ...
        self.render_cache
            .retain(|k| k.select.product != squallar_radar::fields::known::STORM_RELATIVE_VELOCITY);
        true
    }
    ...
    pub fn set_env_heights(
        ...
        self.render_cache.retain(|k| {
            k.select.site != site
                || !squallar_radar::fields::product_for(&k.select.product)
    ...
    pub fn set_melting_layer(
        ...
        self.render_cache.retain(|k| {
            k.select.site != site
                || k.select.product != squallar_radar::fields::known::HYDROMETEOR_CLASSIFICATION
        });
        true
    }
    ...
    pub fn set_storm_motion(
        ...
        self.render_cache.retain(|k| {
            k.select.site != site
                || k.select.product != squallar_radar::fields::known::STORM_RELATIVE_VELOCITY
        });
        true
    }
    ...
    pub fn reset_panes_for_tilts(
        ...
        let hit = self.invalidate_panes_where(site, gui, |product, elevation| {
            ...
            angles
                .iter()
                .any(|a| (a - elevation).abs() <= squallar_egui::pane::ELEVATION_TOLERANCE)
        });
            ...
            let matches = gui.pane(idx).is_some_and(|p| p.site() == site)
                && gui
                    ...
                    .and_then(|(id, elevation)| {
                        Some((squallar_radar::fields::product_for(&id)?, elevation))
                    })
...
impl RenderDispatcher {
    ...
    /// **Every whole-picture overlay raster this application holds a live
    /// plan for**, as `(count, bytes)` over the record's `(pane, layer)`
    /// keys at the size the planner was handed.
    ///
    /// [`Self::overlay_picture_sizes`] answers a different question and is
    /// not this one folded up: it is **one entry per pane**, the size that
    /// pane's pictures are, which is what a surface check holds a bracket's
    /// uploaded bytes against. A pane showing thirteen texture layers is one
    /// entry there and thirteen pictures here, and the page heap carries the
    /// thirteen. Reading the pane figure as the picture load is how a scene
    /// needing 557 MiB of pictures was fitted as though it needed 40, and
    /// `overlay pictures:` now carries both so neither can be mistaken for
    /// the other.
    ///
    ...
    pub(crate) fn resident_overlay_pictures(&self) -> (usize, u64) {
        self.last_overlay_dispatch
            .values()
            .fold((0usize, 0u64), |(n, bytes), req| {
                (
                    n + 1,
                    bytes.saturating_add(
                        u64::from(req.texture.width) * u64::from(req.texture.height) * 4,
                    ),
                )
            })
    }
    ...
    pub(crate) fn set_storm_motion_choice_default(
        ...
        self.set_storm_motion_choice(motion, squallar_radar::srv::SrvFallback::default())
    }

    /// Which derived rung a Level II render's payload should carry.
    pub(crate) fn srv_fallback(&self) -> squallar_radar::srv::SrvFallback {
    ...
    pub(crate) fn melting_layer_product_for(
        ...
        squallar_radar::scan::names_same_volume(cached.volume_start, volume_start)
    ...
    pub fn try_spawn_level3_render(
            ...
            self.spawn_render(
                ...
                squallar_worker::offload::Job::Described(
                    squallar_worker::offload::JobRequest::describe(
                        squallar_radar::jobs::Level3PairJob {
        ...
        self.spawn_render(
            ...
            squallar_worker::offload::Job::Described(
                squallar_worker::offload::JobRequest::describe(
                    squallar_radar::jobs::Level3Job {
                        bytes: std::sync::Arc::clone(&l3_msg.bytes),
                        product,
                        radar_lat: lat,
                        radar_lon: lon,
                    },
                    squallar_worker::offload::ceiling_only_geometry(ceiling_for_this_render),
                ),
            ),
        );
        true
    ...
    pub fn spawn_level2_render(
        ...
        let extracted = cached.or_else(|| {
            squallar_radar::render_input::RenderInput::extract(
                &data,
                elevation,
                product,
                lat,
        ...
        let job = match extracted {
            Some(input) => {
                squallar_worker::offload::Job::Described(
                    squallar_worker::offload::JobRequest::describe(
                        squallar_radar::jobs::RadarPlanJob {
    ...
    fn extract_cache_lookup(
        &mut self,
        key: &ExtractKey,
    ) -> Option<Arc<squallar_radar::render_input::RenderInput>> {
    ...
    pub fn spawn_section_render(
        ...
        pane_idx: usize,
        target: &squallar_egui::pane::SectionTarget,
        extract: impl FnOnce() -> Option<squallar_radar::render_input::RenderInput>,
        ...
        squallar_worker::offload::offload_job("section-render", job, move |output| {
            let _guard = guard;
            // An output of another kind becomes `None` — "nothing to draw" — which the
            // receiver already handles.
            let section = output
                .and_then(|out| out.take::<squallar_radar::xsect::CrossSection>())
                .map(Box::new);
    ...
    pub fn spawn_voxel_build(
        ...
        let job = squallar_worker::offload::Job::Described(squallar_worker::offload::JobRequest {
            job,
            // A voxel grid's shape is already on the wire, so its envelope
            // carries no ceiling — the same effective 0 it has always had.
            geometry: squallar_worker::offload::ceiling_only_geometry(0),
        });
        squallar_worker::offload::offload_job("voxels", job, move |output| {
            let _guard = guard;
            // An output of another kind is `None` — "nothing to draw".
            let grid = output
                .and_then(|out| out.take::<squallar_radar::voxel::VolumeGrid>())
                .map(Box::new);
