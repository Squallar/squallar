...
/// `squallar_egui::pane::elevation_tenths` — because the acceptance check that
...
pub(crate) fn elevation_key(elevation: f32) -> i32 {
    squallar_egui::pane::elevation_tenths(elevation)
}
...
pub(crate) fn render_cache_key(
    ...
    let tilt_selects = squallar_radar::fields::product_for(product)
...
pub(crate) fn radar_field(id: &FieldId) -> Option<squallar_radar::types::RadarProduct> {
    squallar_radar::fields::product_for(id)
}

/// The id the radar layer registers `product` under — the C1 projection.
pub(crate) fn field_id_of(product: squallar_radar::types::RadarProduct) -> FieldId {
    squallar_radar::fields::spec(product).id.clone()
}
