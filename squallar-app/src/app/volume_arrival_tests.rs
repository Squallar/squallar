...
use squallar_egui::CurrentVolumeStamp;
...
fn give_it_a_painter(app: &mut App) {
    app.volume_painter = Some(Arc::new(
        squallar_volumetric::bridge::BridgeVolumePainter::new(
            Arc::clone(&app.volume_store),
            squallar_device_profile::quality::VolumeQuality::BEST,
            app.budgets.offscreen_bytes,
            squallar_volumetric::VolumeSupport::Supported,
        ),
    ));
}
...
/// fixtures to expose.
fn field() -> squallar_source::product::FieldId {
    let field = squallar_radar::fields::known::VELOCITY;
    assert!(
        squallar_radar::fields::spec_for(&field)
...
fn volume_pane_on(
    ...
    pane_idx: usize,
    site: &str,
    field: &squallar_source::product::FieldId,
) {
...
fn published_field(app: &App, pane_idx: usize) -> Option<squallar_source::product::FieldId> {
...
fn a_volume_arriving_produces_the_ask_the_draw_time_trigger_would_have_made() {
    ...
    let region = squallar_egui::pane::VolumeRegion::new(
...
fn an_arrival_is_refused_by_a_pane_playing_a_3d_loop() {
    ...
    let want = expected_target(&app, 0, 6, &squallar_radar::fields::known::REFLECTIVITY);
    assert_eq!(
        app.arrived_volume_asks(&arrival(SITE, 6)).len(),
        1,
    ...
    let mut playing = radar_layer::begin_loop(
        3600,
        squallar_radar::sites::get_radar_site(SITE).expect("a known fixture site"),
        squallar_radar::types::RenderView::Volume,
    );
    playing.frames = vec![squallar_egui::pane::LoopFrame {
        timestamp: at(2),
        image: Some(squallar_egui::pane::LoopFrameImage::Volume(
