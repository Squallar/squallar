...
use squallar_radar::level3::{Level3Product, ProductStamp};
use squallar_radar::loop_downloads::{L3FrameState, LoopDownloadManager};
...
const L3_ID: squallar_source::product::FieldId = squallar_radar::fields::known::ECHO_TOPS;
const L2_ID: squallar_source::product::FieldId = squallar_radar::fields::known::REFLECTIVITY;
...
/// builds one.
fn plan(n: u32) -> squallar_radar::loop_downloads::FramePlan {
    squallar_radar::loop_downloads::FramePlan::new(SITE.to_string(), (0..n).map(ts).collect())
}
...
fn loop_for(product: RadarProduct, n: u32) -> LayerTimeState {
    ...
    ls.frames = (0..n)
        ...
        .collect();
    ls.retarget_renders(&squallar_radar::fields::spec(product).id, 0.5);
    ls
}
...
fn a_level3_loops_batch_settles_on_its_pairings_not_on_volumes() {
    ...
    ls.frames[0].image = Some(squallar_egui::pane::LoopFrameImage::PlanView(image()));
    ls.frames[2].image = Some(squallar_egui::pane::LoopFrameImage::PlanView(image()));
    mgr.cache_l3_product(SITE, code, ts(1), None);
...
fn image() -> squallar_egui::pane::RadarImageData {
    ...
    squallar_egui::pane::RadarImageData {
        ...
        lon: -97.27,
        max_range_km: 100.0,
        placed: squallar_radar::types::ImageBounds::from_radar_site(35.33, -97.27, 100.0).into(),
        nyquist_ms: None,
