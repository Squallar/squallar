...
use squallar_egui::pane::{PaneKind, SectionLine, SectionUnavailable};
...
fn state(app: &crate::app::App) -> &squallar_egui::pane::CrossSectionPane {
    app.gui
        .pane(0)
        .unwrap()
        .cross_section()
...
fn a_volume_with_no_sealed_sweep_yet_is_named_rather_than_blamed_on_the_product() {
    ...
    assert!(
        squallar_radar::current::resolve(None, Some((&*started).into())).is_some(),
...
fn a_product_with_no_vertical_structure_says_so_and_stops_asking() {
    ...
    assert_eq!(
        ...
        Some(SectionUnavailable::ProductHasNoVerticalStructure(
            squallar_radar::fields::known::ECHO_TOPS
        )),
    );
    assert!(
    ...
    let message =
        SectionUnavailable::ProductHasNoVerticalStructure(squallar_radar::fields::known::ECHO_TOPS)
            .message();
...
fn an_override_edit_invalidates_the_srv_vertical_views() {
    ...
    let stale_section = squallar_egui::pane::SectionTarget {
        volume: squallar_egui::pane::VolumeStamp {
            site: SITE.to_owned(),
            collected: volume_time(),
        },
        product: squallar_radar::fields::known::STORM_RELATIVE_VELOCITY,
        line: line(),
        ladder: 7,
    };
    let srv_target = squallar_egui::pane::VolumeTarget {
        volume: squallar_egui::pane::VolumeStamp {
            site: SITE.to_owned(),
            collected: volume_time(),
        },
    ...
    let arm = |app: &mut crate::app::App| {
        ...
        app.volume_store.insert(
            0,
            srv_target.clone(),
            squallar_volumetric::bridge::VolumeEntry::Refused("the old vector's".into()),
        );
    };
    arm(&mut app);
    assert!(
...
fn a_volume_with_nothing_to_cut_is_named_rather_than_waited_on() {
    ...
    assert_eq!(
        ...
        Some(SectionUnavailable::ProductMissingFromVolume(
            squallar_radar::fields::known::STORM_RELATIVE_VELOCITY
        )),
...
fn a_section_of_a_derived_product_is_cut_rather_than_refused_by_name() {
    for product in [
        ...
        assert!(
            squallar_radar::sampler::samplable(product).is_none(),
...
fn a_storm_motion_edit_re_derives_the_cross_section() {
    ...
    let drag = |app: &mut crate::app::App, speed: f32, direction: f32, enabled: bool| {
        app.gui.storm_motion_override = squallar_egui::StormMotionOverride {
            enabled,
            speed_kt: speed,
...
fn blank_cut() -> Box<squallar_radar::xsect::CrossSection> {
...
fn a_new_volume_makes_the_section_on_screen_stale_with_no_reset_arm() {
    ...
    app.gui
        .pane_mut(0)
        .unwrap()
        .set_selected_product(squallar_radar::fields::known::VELOCITY);
