...
const PRODUCT_ID: squallar_source::product::FieldId = squallar_radar::fields::known::ECHO_TOPS;
...
fn a_placed_render_describes_what_it_depicts() {
    ...
    assert_eq!(
        app.gui.pane(0).unwrap().stale_image_on_screen(),
        Some((squallar_radar::fields::known::REFLECTIVITY, 0.5)),
    );
}

#[test]
fn a_long_range_render_is_placed_at_the_size_it_was_rendered_at() {
    let side = squallar_device_profile::constants::LONG_RANGE_IMAGE_SIZE;
    let ctx = egui::Context::default();
