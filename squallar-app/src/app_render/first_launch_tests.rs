...
use squallar_radar::catalogue::{CataloguePosition, SiteCatalogue};
...
fn a_returning_user_without_a_catalogue_still_gets_the_network_this_session() {
    ...
    assert!(
        !squallar_radar::sites::knows_site(RETURNING),
    ...
    assert!(
        squallar_radar::sites::get_radar_site(RETURNING).is_some(),
...
fn a_first_launch_adopts_its_first_catalogue_without_waiting_for_the_next_one() {
    ...
    assert!(
        squallar_radar::sites::get_radar_site(ARRIVAL).is_none()
            && !squallar_radar::sites::knows_site(UNPLACEABLE),
    ...
    let row = squallar_radar::sites::get_radar_site(ARRIVAL)
        .expect("the catalogue's placed radar must reach the live table now");
    assert_eq!((row.lat, row.lon), (-30.0, -140.0));
    assert!(
        squallar_radar::sites::unplaced().contains(&UNPLACEABLE),
...
fn a_launch_that_already_knows_the_network_still_waits_for_the_next_one() {
    ...
    assert!(
        !squallar_radar::sites::knows_site(LATER),
...
fn a_catalogue_that_cannot_be_persisted_is_still_applied_to_this_session() {
    ...
    assert!(
        squallar_radar::sites::get_radar_site(UNSTORABLE).is_some(),
...
fn a_volume_that_places_its_own_radar_names_it_in_the_same_breath() {
    ...
    let mut app = headless(TestBridge::desktop());
    assert!(
        !squallar_radar::sites::knows_site(TAUGHT),
...
fn the_catalogue_landing_draws_the_volume_the_launch_could_not_place() {
    ...
    assert!(
        !squallar_radar::sites::knows_site(BLIND),
        "precondition: nothing may place this radar yet, or this proves nothing",
    );
    ...
    assert_eq!(
        info.site.name,
        squallar_radar::sites::UNKNOWN_SITE_NAME,
    ...
    app.gui
        .apply(squallar_egui::shell_api::GuiEvent::ScanInfoForSite {
    ...
    assert_eq!(
        super::one_render_per_sweep_tests::asked_for(&recorded),
        vec![(squallar_radar::types::RadarProduct::Reflectivity, 0.5)],
