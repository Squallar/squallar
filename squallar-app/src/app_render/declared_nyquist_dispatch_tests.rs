...
use squallar_worker::offload::{JobRequest, JobSink};
...
fn declared() -> Arc<squallar_radar::nyquist::DeclaredNyquist> {
    Arc::new(
        [(1, DECLARED_MS), (2, OTHER_CUT_MS)]
            .into_iter()
            .collect::<squallar_radar::nyquist::DeclaredNyquist>(),
    )
}
...
fn app_showing_site() -> crate::app::App {
    ...
    product_elevations.insert(RadarProduct::NormalizedRotation, vec![0.5]);
    {
        ...
        pane.set_selected_product(squallar_radar::fields::known::NORMALIZED_ROTATION);
...
fn declaration_on_the_wire(job: &JobRequest) -> Vec<(u8, f64)> {
    let input = job
        ...
        .or_else(|| {
            job.job
                .downcast_ref::<squallar_radar::jobs::SectionJob>()
...
fn a_plan_view_and_a_section_of_one_sweep_fold_at_the_same_speed() {
    ...
    squallar_worker::offload::set_worker(Box::new(Recorder(Arc::clone(&posted))));
    ...
    squallar_worker::offload::abandon_worker("test teardown");
...
fn a_loop_frame_and_the_still_frame_beside_it_fold_at_the_same_speed() {
    ...
    let site = squallar_radar::sites::get_radar_site(SITE).expect("KTLX is a real radar");
    let params = crate::render_dispatch::RenderParams {
        product: squallar_radar::types::RadarProduct::NormalizedRotation,
        elevation: 0.5,
        lat: site.lat,
        lon: site.lon,
    };
    ...
    assert!(
        app.spawn_loop_frame_render(
            ...
            volume_time(),
            params,
            squallar_egui::pane::RenderTarget {
                site: SITE.to_string(),
                product: squallar_radar::fields::known::NORMALIZED_ROTATION,
                elevation: 0.5,
            },
        ),
