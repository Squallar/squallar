...
fn recorder() -> (
    ...
    squallar_worker::offload::InstalledTestWorker,
) {
...
fn declared() -> Arc<squallar_radar::nyquist::DeclaredNyquist> {
    let mut table = squallar_radar::nyquist::DeclaredNyquist::empty();
...
fn an_arrival_populates_only_the_sites_shown_panes() {
    ...
    let site_radar = squallar_radar::sites::get_radar_site(SITE).unwrap();
    let fresh = squallar_radar::render_input::RenderInput::extract(
        &sample_scan(200),
        TILT,
    ...
    .with_srv_fallback(squallar_radar::srv::SrvFallback::default())
...
fn a_second_arrival_never_serves_the_previous_snapshots_payload() {
    ...
    let fresh = squallar_radar::render_input::RenderInput::extract(
        ...
        TILT,
        RadarProduct::Reflectivity,
        squallar_radar::sites::get_radar_site(SITE).unwrap().lat,
        squallar_radar::sites::get_radar_site(SITE).unwrap().lon,
        None,
        None,
    )
    .expect("the fixture extracts")
