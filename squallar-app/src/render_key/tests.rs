...
fn the_radar_key_is_the_same_in_dark_and_light() {
    let overlays = OverlayRegistry::with_handlers(squallar_egui::sources::all());
...
fn the_elevation_part_is_present_exactly_when_the_tilt_selects_the_picture() {
    ...
    for &view in RenderView::all() {
        for product in squallar_radar::fields::known::ALL.iter() {
...
fn the_key_and_the_acceptance_check_bucket_a_tilt_the_same_way() {
    ...
    for a in angles {
        assert_eq!(
            elevation_key(a),
            squallar_egui::pane::elevation_tenths(a),
