...
use squallar_location::coordinate_for_timezone;
...
pub fn site_for_timezone(zone: &str) -> Option<&'static str> {
    ...
    squallar_radar::sites::nearest_wsr88d_site(lat, lon).map(|(site, _)| site.name)
}
...
mod tests {
    ...
    fn an_anchor_skips_the_tdwr_and_the_test_bed() {
        install_radars();
        let (site, _) = squallar_radar::sites::nearest_wsr88d_site(35.4676, -97.5164)
    ...
    fn a_process_with_no_radars_offers_no_hint() {
        let empty = squallar_radar::sites::build_table(std::iter::empty());
    ...
    /// `squallar_radar::catalogue`'s live tests are. Run it when an anchor is
    /// added or moved:
    ///
    /// ```text
    /// cargo test -p squallar-app --all-features -- --ignored the_live_anchor_survey
    /// ```
    ///
    ...
    fn the_live_anchor_survey() {
        ...
        let catalogue = tokio::runtime::Runtime::new()
            .expect("a runtime")
            .block_on(squallar_radar::catalogue::fetch(&sources))
            .expect("the live catalogue");
        let table = squallar_radar::sites::build_table(catalogue.fixes());
        assert!(
            table.rows().len() > 150,
        ...
        for anchor in squallar_location::ZONE_ANCHORS {
            let (site, dist) = table
