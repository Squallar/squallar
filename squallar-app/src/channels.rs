...
pub struct RenderedImage {
    ...
    /// [`squallar_radar::hover::HoverSource`]. The same numbers at the resolution
...
///
/// Carries the [`SectionTarget`](squallar_egui::pane::SectionTarget) rather than a
/// pane index: a section takes far longer to produce than the user takes to draw
/// another line over it.
pub struct SectionResponse {
    pub pane_idx: usize,
    pub generation: u64,
    pub target: squallar_egui::pane::SectionTarget,
    ...
    pub section: Option<Box<squallar_radar::xsect::CrossSection>>,
}

/// Result from a background voxel build. Carries the
/// [`VolumeTarget`](squallar_egui::pane::VolumeTarget) and no pane index: the
...
pub struct VoxelResponse {
    ...
    pub grid: Option<Box<squallar_radar::voxel::VolumeGrid>>,
}
...
pub struct OverlayRenderResponse {
    ...
    /// `squallar_overlays::render::rasterize::alpha_tests` — and
    ...
    ///
    /// [`LayerTimeState::frame_at_stamp_mut`]: squallar_egui::pane::LayerTimeState::frame_at_stamp_mut
    pub frame: Option<squallar_source::time::FrameStamp>,
}
...
pub struct LoopL3ListResponse {
    ...
    /// [`squallar_radar::level3::candidates_near`]'s job. An empty list is cached
...
pub struct LoopSectionResponse {
    ...
    pub key: squallar_egui::pane::SectionLoopKey,
    /// The fingerprint of the tilt ladder this raster was cut from, from
    /// `squallar_radar::sampler::ladder_fingerprint`. Set on the failure path too.
    pub ladder: u64,
    ...
    pub axes: Option<squallar_radar::xsect::SectionAxes>,
    pub tilt_elevations_deg: Vec<f64>,
    /// When each of those rungs was flown, ms since the Unix epoch, in the same
    /// order — `squallar_radar::xsect::CrossSection::tilt_collected_ms`.
...
pub struct ChunkResponse {
    ...
    pub generation: u64,
    pub site: String,
    pub poller: Box<squallar_radar::chunks::ChunkPoller>,
    pub result: Result<squallar_radar::chunks::PollOutcome, String>,
}

/// The environmental 0 °C / −20 °C heights over a site, from Open-Meteo —
/// fetched when a scan loads, TTL-gated by
/// [`squallar_radar::sounding::ENV_HEIGHTS_TTL`].
...
pub struct MeltingLayerResponse {
    ...
    /// volume — an ordinary gap; `squallar_radar::hca::resolve_melting_layer` falls
    /// to its next rung.
...
pub struct SiteCatalogueResponse {
    ...
    pub catalogue: Option<squallar_radar::catalogue::SiteCatalogue>,
}
