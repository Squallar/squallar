...
fn tdwr_chunk_scan_info(products: &[(RadarProduct, &[f32])], minute: u32) -> ScanInfo {
    ...
    ScanInfo {
        site: squallar_radar::sites::get_radar_site(TDWR)
...
fn section_pane_showing_the_wsr88ds_cut(app: &mut crate::app::App) {
    ...
    let line = squallar_egui::pane::SectionLine::new(
    ...
    xsect.rendered_for = Some(squallar_egui::pane::SectionTarget {
...
fn a_pane_that_did_not_change_radar_keeps_its_loop() {
    ...
    let bystander_site = squallar_radar::sites::get_radar_site(BYSTANDER)
    ...
    *pane.time_state_mut(&known::RADAR) = squallar_egui::radar_layer::begin_loop(
    ...
    app.loop_mgr.set_plan(
        0,
        squallar_radar::loop_downloads::FramePlan::new(WSR88D.to_string(), vec![at(0)]),
    );
    assert!(
        app.loop_mgr
...
fn re_picking_the_site_a_pane_is_on_keeps_its_loop() {
    ...
    let radar_site = squallar_radar::sites::get_radar_site(WSR88D)
...
fn one_grid_bytes() -> usize {
    ...
    let shape = grid.dims();
    squallar_volumetric::raymarch::resident_grid_bytes([
...
fn pane_animating(app: &mut crate::app::App, site: &str, ids: &[LayerId]) {
    ...
    for (n, id) in ids.iter().enumerate() {
        ...
        ls.span_secs = 3600;
        ls.frames = (0..2 + n)
            .map(|i| squallar_egui::pane::LoopFrame {
