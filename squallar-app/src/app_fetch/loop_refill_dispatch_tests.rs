...
use super::*;
use crate::app::App;
use squallar_source::handler::{FetchTask, FrameListingResult, PaneRef};
...
impl squallar_overlays::render::overlay_state::OverlayHandler for RangeRecorder {
    fn id(&self) -> LayerId {
        self.id.clone()
    }
...
fn frame(m: i64) -> squallar_egui::pane::LoopFrame {
...
fn a_clock_the_loop_answers_for_never_asks_however_far_it_travels() {
    ...
    for step in 0..20i64 {
        app.gui
            .pane_mut(0)
            .expect("one pane")
            .set_time_mode(squallar_egui::pane::TimeMode::AsOf(ts(60 + step)));
...
fn an_absurd_scrub_target_asks_for_one_span_and_not_the_gap() {
    ...
    app.gui
        .pane_mut(0)
        .expect("one pane")
        .set_time_mode(squallar_egui::pane::TimeMode::AsOf(far));
...
fn six_hundred_passes_over_one_hole_issue_one_request() {
    ...
    app.gui
        .pane_mut(0)
        .expect("one pane")
        .set_time_mode(squallar_egui::pane::TimeMode::AsOf(ts(30)));
