...
fn seed(app: &mut crate::app::App, id: &LayerId) {
    use squallar_overlays::render::handlers::{alert, discussion, outlook};
    let data: squallar_overlays::render::overlay_state::FetchPayload = match id {
        id if *id == known::NWS_ALERTS => {
            let alert = squallar_overlays::nws::alert::NwsAlert {
                id: "urn:test".into(),
            ...
            Box::new(alert::NwsAlertFetchResult(Ok(
                squallar_overlays::nws::fetch::ActiveAlerts::whole(vec![alert]),
            )))
        }
        ...
        id if *id == known::SPC_DISCUSSIONS => {
            let md = squallar_overlays::spc::discussion::SpcDiscussion {
                number: 1,
                title: "Mesoscale Discussion #0001".into(),
                text: String::new(),
                link: String::new(),
                md_type: squallar_overlays::spc::discussion::MdType::Convective,
                polygon: vec![vec![
                    (34.2, -98.8),
                    (34.2, -97.2),
                    (35.8, -97.2),
