...
fn target(site: &str, minute: u32, product: RadarProduct, ladder: u64) -> SectionTarget {
    SectionTarget {
        ...
        product: squallar_radar::fields::spec(product).id.clone(),
...
fn a_payload_is_reused_for_another_line_through_the_same_volume() {
    ...
    let key = SectionInputKey::of(&base, None, squallar_radar::srv::SrvFallback::default());
...
fn a_payload_is_not_reused_across_volume_site_moment_or_ladder() {
    ...
    for (other, why) in [
        ...
        assert_ne!(
            SectionInputKey::of(&other, None, squallar_radar::srv::SrvFallback::default()),
            key,
            "{why}"
        );
    }
...
fn the_storm_motion_vector_is_part_of_a_storm_relative_payloads_identity() {
    ...
    assert_ne!(
        slow,
        SectionInputKey::of(&srv, None, squallar_radar::srv::SrvFallback::default()),
    ...
    assert_eq!(
        SectionInputKey::of(&refl, None, squallar_radar::srv::SrvFallback::default()),
        SectionInputKey::of(&refl, None, squallar_radar::srv::SrvFallback::default()),
    );
}
