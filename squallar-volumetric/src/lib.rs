...
use squallar_device_profile::constants::VOLUME_GRID_CELLS;
...
/// `squallar_radar::voxel`'s module doc for what that retired.
///
...
pub const VOLUME_ENV_VAR: &str = "SQUALLAR_VOLUME";
...
/// What a `SQUALLAR_VOLUME` value means.
///
...
mod tests {
    use super::*;
    use squallar_device_profile::constants::WEBGL2_MAX_TEXTURE_DIMENSION_3D;
    ...
    /// `squallar_gpu::device::request_device`), the device grants exactly them,
    ...
    fn the_web_limits_this_app_requests_clear_the_volume_probes_floor() {
        use squallar_gpu::device::device_limits;
