...
//! [`RenderMode::Texture`](rustdar_source::handler::RenderMode) and keeps a
...
//! `rustdar_gpu::egui_renderer::texture_upload::UploadTotals`. **The two are
...
//!   worker transport in `rustdar_web`'s `worker_port`.
...
///
/// **Reported, never gated on, by this crate.** `rustdar-egui` is the UI layer
/// and has no frame of its own to report on; the running-total line is written
/// by `rustdar-app` once a frame, from [`totals`], on the same terms
/// `rustdar_volumetric::degrade` already uses for the surface-loss count. That
