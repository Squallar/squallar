...
//! literal-table pins in `rustdar-worker`, and a double cannot catch a layer
...
//!
//! [`FrameSource`]: rustdar_source::time::FrameSource

use std::any::Any;

use chrono::{Duration, NaiveDateTime};
use rustdar_overlays::render::overlay_state::OverlayRegistry;
use rustdar_source::handler::{FetchConfig, FetchPayload, PaneRef};
...
impl Bench {
    fn new() -> Self {
                ...
                let (config, saved) = match id.as_str() {
                    ...
                    "Gmgsi" => (
                        ...
                        Some(serde_json::json!({
                            "enabled": true,
                            "channel": rustdar_overlays::gmgsi::GmgsiChannel::LongwaveIr.as_str(),
                        })),
                    ),
                    "Mrms" => (
                        serde_json::Value::Null,
                        Some(serde_json::json!({
                            "enabled": true,
                            "product": rustdar_overlays::mrms::MrmsProduct::ReflectivityComposite
                                .as_str(),
                        })),
                    ),
        ...
        registry.apply_frames(
            ...
            Box::new(rustdar_radar::source::RadarListing {
                site: "KTLX".to_string(),
                range: radar_range,
                scans: radar_stamps()
                    .iter()
                    .map(|&valid| {
                        (
                            valid,
                            rustdar_radar::archive::Identifier::new(format!(
                                "KTLX{}",
        ...
        registry.apply_frames(
            ...
            Box::new(rustdar_overlays::gmgsi::GmgsiListing {
                channel: rustdar_overlays::gmgsi::GmgsiChannel::LongwaveIr,
                range: gmgsi_range,
                keys: gmgsi_stamps()
                    .iter()
        ...
        registry.apply_frames(
            ...
            Box::new(rustdar_overlays::mrms::MrmsListing {
                product: rustdar_overlays::mrms::MrmsProduct::ReflectivityComposite,
                range: mrms_range,
                keys: mrms_stamps()
                    .iter()
...
/// `rustdar-overlays`' own run selection, reached through its public door
...
fn latest_run(now: NaiveDateTime) -> NaiveDateTime {
    let (date, hour) = rustdar_overlays::hrrr::fetch::run_for(now);
...
fn every_stop_a_pane_can_make_is_inside_what_the_layer_asked_to_hold() {
    ...
    assert_eq!(
        ...
        "nine of this build's fifteen layers read the depicted instant",
    );
}
        ...
        if handler.time_axis() == TimeAxis::Live {
            ...
            assert!(
                ...
                "{name} reads the depicted instant and asked for nothing over \
                 ...
                 answer that is right for the six `Live` layers must not be \
