...
//!
//! The site comes out of the volume, not out of a list: `squallar-radar` carries
...
pub fn scan_from_archive(path: &std::path::Path) -> Scan {
    ...
    assert!(
        ...
        "{} is gzipped. Level II reaches this crate through nexrad-data's \
         bzip2-per-record framing and nothing in squallar-app's dependency \
...
pub fn site_of(scan: &Scan, path: &std::path::Path) -> (String, f64, f64) {
    ...
    let row = squallar_radar::sites::resolve([(name.as_str(), SiteFix::Learned(position))])
        .get(&name)
