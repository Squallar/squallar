...
//! nothing about how squallar should look; it exists so a projection matrix
...
//! OUT=/tmp/rd_kdmx \
//! cargo test -p squallar-gpu --test volume_real_mask -- --ignored --nocapture
//! ```
//!
...
//!
//! `.gz` volumes are refused rather than mis-decoded: `squallar-radar` reaches
...
fn render_a_real_volume_mask() {
    ...
    uniform.extinction_per_km = squallar_volumetric::uniform::DEFAULT_EXTINCTION_PER_KM;
    ...
    uniform.reconstruction_lod =
        squallar_volumetric::bridge::cloud_reconstruction_lod_for(largest_cell_km);
    ...
    uniform.empty_index_threshold =
        squallar_volumetric::bridge::empty_index_threshold_for(grid.fade_band());
    ...
    let floor_pixels = squallar_radar::render::find_closest_elevation(
        &scan,
        squallar_radar::types::RadarProduct::Reflectivity,
        0.0,
    )
    .and_then(|elevation| {
        squallar_radar::render::render_radar_to_image(
            &scan,
            elevation,
            squallar_radar::types::RadarProduct::Reflectivity,
            site_lat,
    ...
    .and_then(
        |squallar_radar::render::SweepRender {
             mut image,
            ...
            // harness was written.
            let bounds =
                squallar_radar::types::ImageBounds::from_radar_site(site_lat, site_lon, extent_km);
    ...
    let meta = format!(
        ...
        size[0],
        size[1],
        squallar_volumetric::uniform::DEFAULT_EXTINCTION_PER_KM,
...
/// application asks it.
fn production_shape() -> squallar_radar::voxel::VoxelShape {
    let instance =
...
fn device() -> (wgpu::Device, wgpu::Queue) {
    ...
    pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor {
        label: Some("squallar.volume.mask.device"),
...
fn read_back(
    ...
    let staging = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("squallar.volume.mask.readback"),
...
/// `PRODUCT`, restricted to what `squallar_radar::derive::volume_slot` accepts
...
fn product_from_name(raw: &str) -> RadarProduct {
    ...
    assert!(
        squallar_radar::derive::volume_slot(product).is_some(),
...
fn the_harness_accepts_every_product_the_vertical_views_render() {
    ...
    for product in squallar_radar::types::RadarProduct::all() {
        if squallar_radar::derive::volume_slot(*product).is_none() {
            continue;
        }
        assert_eq!(
...
fn motion_from_env(product: RadarProduct) -> squallar_radar::srv::MotionInputs {
    let none = squallar_radar::srv::MotionInputs::default();
    ...
    squallar_radar::srv::MotionInputs {
