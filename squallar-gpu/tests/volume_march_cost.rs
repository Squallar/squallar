...
//! cargo test -p squallar-gpu --test volume_march_cost -- --ignored --nocapture
//! ```
//!
...
fn timed_passes(
    ...
    let queries = device.create_query_set(&wgpu::QuerySetDescriptor {
        label: Some("squallar.volume.cost.queries"),
        ty: wgpu::QueryType::Timestamp,
        count: 2,
    });
    let resolve = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("squallar.volume.cost.resolve"),
        size: 16,
        usage: wgpu::BufferUsages::QUERY_RESOLVE | wgpu::BufferUsages::COPY_SRC,
        mapped_at_creation: false,
    });
    let staging = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("squallar.volume.cost.staging"),
        size: 16,
...
fn device_with_timestamps() -> (wgpu::Device, wgpu::Queue) {
    ...
    pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor {
        label: Some("squallar.volume.cost.device"),
...
fn read_back(
    ...
    let staging = device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("squallar.volume.cost.readback"),
