...
//!
//! ```text
//! cargo test -p squallar-gpu --test volume_gpu -- --ignored --nocapture
//! ```
//!
...
macro_rules! clearing_pass {
    ($encoder:expr, $view:expr) => {
        $encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("squallar.volume.test.pass"),
...
fn an_isosurface_paints_an_opaque_lit_surface_from_the_data_alone() {
    ...
    uniform.iso_threshold = squallar_volumetric::uniform::ISO_OFF;
    let pixels = raymarch_once(
        &device, &queue, &pipelines, cells, &filled, &lut, &uniform, size,
    );
    assert_eq!(
...
fn an_isosurface_at_the_shipped_rung_keeps_its_sub_kernel_features() {
    ...
    println!(
        ...
        size[1],
        raw.0,
        squallar_volumetric::bridge::CLOUD_RECONSTRUCTION_LOD,
        smoothed.0,
        raw.1,
        squallar_volumetric::bridge::CLOUD_RECONSTRUCTION_LOD,
        smoothed.1,
...
fn an_omitted_coarse_level_marches_the_raw_field_at_the_cloud_rung() {
    ...
    const CLOUD_LOD: f32 = squallar_volumetric::bridge::CLOUD_RECONSTRUCTION_LOD;
...
/// ```text
/// cargo test -p squallar-gpu --test volume_gpu \
...
fn the_charged_grid_bytes_are_never_under_what_the_device_reserved() {
    use squallar_volumetric::raymarch::grid_bytes_at;
    ...
    let reserved = |device: &wgpu::Device| -> u64 {
        device
            ...
            .allocations
            .iter()
            .filter(|a| a.name == "squallar.volume.grid")
            .map(|a| a.size)
            .sum()
    };
...
fn probe_mip_layout(device: &wgpu::Device, queue: &wgpu::Queue) -> MipLayout {
    use squallar_volumetric::VOLUME_TEXTURE_FORMAT;

    const PROBE_CELLS: [u32; 3] = [128, 128, 64];
    const PROBE_LABEL: &str = "squallar.volume.grid.miplayout.probe";
