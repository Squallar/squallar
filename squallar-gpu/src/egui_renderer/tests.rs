...
// `the_frame_path_submits_only_through_prepared_frame` pins squallar-app's
...
// this crate's `egui_renderer.rs` and squallar-app's `app_state.rs` (the
    ...
    impl egui_wgpu::CallbackTrait for SentinelCallback {
        fn prepare(
            ...
            let mut own = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("squallar.volume.test.sentinel"),
            });
