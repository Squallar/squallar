...
fn the_device_side_cap_is_floored_at_the_guarantee_and_raised_above_it() {
    let budget = squallar_device_profile::constants::VOLUME_MIRROR_BYTES_MAX;
    assert_eq!(
    ...
    for arm in squallar_device_profile::budget::BudgetLimits::SHIPPED {
        let budgets = squallar_device_profile::budget::resolve(
            &squallar_device_profile::budget::DeviceProfile {
                limits: arm,
