...
// `test_chunk_identifier_next_chunk_intermediate` was here. It builds an
// `ElevationChunkMapper` from a real VCP message read out of
// `include_bytes!("../../downloads/KDMX20220305_232324_V06")`, a file that
// exists only in upstream's checkout after a download step and is not in the
// published tarball, so it cannot compile here. Unlike the fixture in
// `error_handling.rs` it cannot be synthesised either -- it needs a genuine
// message 5 with real elevation cuts. Removed rather than the whole file,
// which is why the other thirty-odd tests in it still run. See VENDORED.md.
