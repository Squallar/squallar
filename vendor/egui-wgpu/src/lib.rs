...
// LOCAL CHANGE (squallar, see VENDORED.md): the `cfg` upstream put on the
// sibling assertion in `renderer.rs` and not on this one. `WgpuConfiguration`
// holds `wgpu` handles, which are `Send + Sync` only where wgpu's own
// `send_sync` alias holds -- so on wasm32 without
// `fragile-send-sync-non-atomic-wasm` this test cannot compile, and a
// workspace that builds this crate's test target for wasm32 sees 51 errors
