...
/// Where a frame's index and vertex bytes are written on their way to the GPU.
///
/// **This trait is not upstream.** See `VENDORED.md` next to this crate.
///
/// [`Renderer::update_buffers`] stages both arrays through
/// [`wgpu::Queue::write_buffer_with`], which hands back a mapping of a
/// `MAP_WRITE | COPY_SRC` buffer. `wgpu-hal` maps that usage pair to
/// `gpu_allocator`'s `MemoryLocation::CpuToGpu`, which on a discrete card is
/// the host-visible BAR window: every host store is a write down the `PCIe` link
/// and the frame thread stands and waits for the last one. Measured on an
/// RTX 3090 under Vulkan, 1 MiB to 30 MiB, five runs, minimum:
/// **2.15 GB/s through that window against 21 GB/s into ordinary cached
/// system RAM** — a ~10x per-byte penalty, paid on the frame thread, on every
/// byte of every frame.
///
/// A stager is somewhere better to put those bytes. It is handed the two
/// destination buffers with the byte count each needs, and a closure that
/// fills **one contiguous region**: the index bytes at `0`, then the vertex
/// bytes at `index.1`. Both counts are multiples of four, so the vertex half
...
/// A boxed [`GeometryStager`], with the same `Send + Sync` bounds
/// [`CallbackResources`] is chosen by and for the same reason — a `Renderer`
/// that carries one must stay as `Send + Sync` as it was without one, which
/// upstream's `renderer_impl_send_sync` asserts under exactly this `cfg`.
#[cfg(not(all(
    target_arch = "wasm32",
    not(feature = "fragile-send-sync-non-atomic-wasm"),
)))]
pub type BoxedGeometryStager = Box<dyn GeometryStager + Send + Sync>;

/// A boxed [`GeometryStager`]. See the other arm of this alias.
#[cfg(all(
    target_arch = "wasm32",
    not(feature = "fragile-send-sync-non-atomic-wasm"),
))]
pub type BoxedGeometryStager = Box<dyn GeometryStager>;
...
/// make, and the order [`Renderer::render`] reads the slice lists back in.
fn meshes(paint_jobs: &[epaint::ClippedPrimitive]) -> impl Iterator<Item = &epaint::Mesh> + Clone {
    paint_jobs
        .iter()
...
/// Where each mesh's bytes land in one of the two buffers, in mesh order.
/// Pure arithmetic, split out of the copy loops so that the queue route and a
/// [`GeometryStager`] share one definition of where a mesh goes.
fn fill_slices(slices: &mut Vec<Range<usize>>, sizes: impl Iterator<Item = usize>) {
    slices.clear();
    let mut offset = 0;
    for size in sizes {
...
pub struct Renderer {
    ...
    /// **Not upstream.** Where [`Self::update_buffers`] puts a frame's index
    /// and vertex bytes when something better than the queue's BAR mapping is
    /// installed. `None` — the default, and every platform without one — is
    /// upstream's behaviour exactly. See [`GeometryStager`].
    geometry_stager: Option<BoxedGeometryStager>,
...
impl Renderer {
    ...
    /// **Not upstream.** Put this renderer's index and vertex staging somewhere
    /// other than the queue's BAR mapping. See [`GeometryStager`], which says
    /// what the BAR costs and what the contract is.
    pub fn set_geometry_stager(&mut self, stager: BoxedGeometryStager) {
        self.geometry_stager = Some(stager);
    }
    ...
    pub fn update_buffers(
        ...
        //
        // Upstream sized, grew and filled each buffer in one block, with the
        // slice list built inside the copy loop. The three are separated here
        // because a `GeometryStager` claims **one** region for both arrays and
        // so has to be told both sizes before either is written, and because
        // the slice arithmetic is the same whichever route the bytes take.
        // The sizes, the growth rule and the slice contents are upstream's,
        // unchanged: what moved is only where the bytes are copied to.
        ...
        if index_count > 0 && self.index_buffer.capacity < required_index_buffer_size {
        ...
        if vertex_count > 0 && self.vertex_buffer.capacity < required_vertex_buffer_size {
        ...
        // Guarded exactly as upstream guarded the blocks these came out of: a
        // frame with no indices leaves the index slice list as it was, which is
        // what `render` then reads.
        if index_count > 0 {
            fill_slices(
                &mut self.index_buffer.slices,
                meshes(paint_jobs).map(|mesh| mesh.indices.len() * std::mem::size_of::<u32>()),
            );
        }
        if vertex_count > 0 {
            fill_slices(
                &mut self.vertex_buffer.slices,
                meshes(paint_jobs).map(|mesh| mesh.vertices.len() * std::mem::size_of::<Vertex>()),
            );
        }

        // Destructured so the stager, the two buffers and the slice lists are
        // disjoint borrows of `self` rather than one whole-struct borrow.
        let Self {
            index_buffer,
            vertex_buffer,
            geometry_stager,
        ...
        let staged = geometry_stager.as_mut().is_some_and(|stager| {
            ...
            stager.stage(
                ...
                &mut |region| {
                    ...
                    let base = required_index_buffer_size as usize;
                    for (mesh, slice) in meshes(paint_jobs).zip(&vertex_buffer.slices) {
                        region
                            .slice(base + slice.start..base + slice.end)
