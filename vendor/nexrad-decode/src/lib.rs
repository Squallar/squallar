...
#![forbid(unsafe_code)]
// VENDORED DEVIATION (the only source edit in this directory; see
// ../VENDORED.md). Upstream denies these unconditionally, and its own test
// code -- `segmented_slice_reader`'s unit tests and all 15 `snapshot_test`
// modules -- breaks the rule 23 times. Upstream never notices because it lints
// without `--all-targets`, so `cfg(test)` code is never compiled under clippy;
// this workspace's CI lints `--all-targets` and the crate would not build under
// clippy at all. A manifest `[lints]` allow cannot fix it: a `#![deny]`
// attribute outranks any command-line level. Scoping the deny to non-test
// builds keeps it saying what it means -- no unwrap in the library -- and is
// the form to offer upstream.
