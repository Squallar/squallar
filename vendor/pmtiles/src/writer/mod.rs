...
// VENDORED: was also `#[cfg(feature = "mmap-async-tokio")]`; see async_reader.rs.
#[cfg(test)]
...
mod tests {
    ...
    use crate::{
        AsyncPmTilesReader, Compression, PmTilesWriter, SliceBackend, TileCoord, TileId, TileType,
    };
    ...
    async fn roundtrip_raster() {
        ...
        let backend = SliceBackend::try_from(&path).await.unwrap();
    ...
    async fn verify_entries(file_path: &str, num_tiles: u64) {
        let backend = SliceBackend::try_from(file_path).await.unwrap();
