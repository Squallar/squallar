...
/// latitude and the latitude term sets the bound. Far tighter than the 3 km
/// spacing: the failure mode guarded against is systematic displacement.
...
/// **Source (A) — the file's own section 3.** Not sufficient alone: `latlons`
/// forward-projects La1/Lo1 and immediately inverse-projects it, so corner (0,0)
/// is invariant under any change to the earth radius.
#[test]
...
/// **Source (B) — PROJ 9.8.1**, forward-projecting La1/Lo1, stepping 3000 m per
/// index exactly as `latlons` does, and inverse-projecting:
///
/// ```text
/// proj +proj=lcc +a=6371229 +b=6371229 +lat_0=38.5 +lon_0=262.5 \
...
/// All four corners plus an interior point against PROJ, the interior point
/// ~500 m from the projection origin so it pins the origin placement.
#[test]
...
/// correctly: NOMADS re-encoded through wgrib2, re-rounding `Lo1` from 237280472
/// to **237280471** microdegrees. One microdegree is ~10 cm at the anchor but
/// rotates the grid about the cone apex, so at the NE corner the two encodings
/// differ by 1.12e-6° — just over [`HRRR_TOLERANCE_DEG`], hence a separate
/// fixture.
///
/// **Source (B) — PROJ 9.8.1**, same procedure as [`HRRR_CORNERS_FROM_PROJ`]
/// but anchored at Lo1 = 237.280471.
#[test]
...
/// The size in [`LambertGrid`]'s own doc, **measured**. 64-bit only: `usize` is
/// four bytes on wasm32, where the same layout is 96.
#[test]
...
/// The tooltip's O(1) lookup must land where a scan over 1.9 M points would,
/// sampled away from the anchor: `nearest` divides by the same `dx`/`dy`.
#[test]
...
///
/// Under i-consecutive scanning `index_of(ni, j)` is the *west* edge of row
/// `j + 1` — an in-range index holding a reading from 5,000 km away.
#[test]
...
/// [`LambertGrid::index_bounds`] is a closed form over a candidate set, checked
/// against the scan it replaces: every grid point inside the box must fall
/// inside the bounds, and the answer must be *tight* or it degenerates to
/// `0..ni`.
#[test]
...
/// wrongly. Scanning mode 0b0100_0000 is +i west-to-east, +j south-to-north.
#[test]
...
/// The tangent cone constant must come from **Latin1, not LaD**. Every other
/// fixture is HRRR-shaped, where `LaD == Latin1 == Latin2 == 38.5°`, so the two
/// are indistinguishable; here LaD = 45°, Latin1 = Latin2 = 30°.
///
/// **Source (B) — PROJ 9.8.1:**
///
/// ```text
/// proj +proj=lcc +a=6371229 +b=6371229 +lat_0=45 +lon_0=-97.5 \
///      +lat_1=30 +lat_2=30
/// ```
#[test]
...
/// The two branches must be one continuous function. Expected values come from
/// `f64::sin` of the mean parallel, not from the projection.
#[test]
...
/// eq. 15-2 gives the same `F` from either parallel by construction.
#[test]
...
/// tangent on a sphere, so both branches would otherwise ship unverified.
///
/// From the guidance note: Clarke 1866 (a = 6378206.400 m, 1/f = 294.97870);
/// false origin 27°50'00"N, 99°00'00"W; standard parallels 28°23'00"N and
/// 30°17'00"N; false easting 2000000.00 US survey feet, northing 0.00. Forward
/// for 28°30'00"N, 96°00'00"W: E = 2963503.91, N = 254759.80 US survey feet.
///
...
/// (293676.5791, 77650.9423) m, so (B) and (C) corroborate.
#[test]
...
/// Without this, the ellipsoidal machinery could be dead code and the EPSG test
/// would still pass. Measured separation at the EPSG test point is ~332 m in
/// northing; the 100 m bound is a floor, not a fitted value.
#[test]
...
/// A southern-hemisphere cone gives `n < 0`, which flips the sign conventions in
/// Snyder eq. 15-11 and 14-11. Untested, that path silently mirrors the grid.
///
/// **Source (B) — PROJ 9.8.1**, forward direction:
///
/// ```text
/// proj +proj=lcc +a=6371229 +b=6371229 +lat_0=-38.5 +lon_0=145 \
///      +lat_1=-30 +lat_2=-60
/// ```
#[test]
...
/// Degenerate parameter sets must be rejected, not silently produce NaN grids.
#[test]
fn degenerate_parameters_are_rejected() {
    ...
    // Unknown earth-shape code must surface as an error, not a panic.
    let mut grid = hrrr_conus_grid();
...
/// `(lon + 540) % 360 - 180` returns out-of-range values below -540°, where
/// `rem_euclid` gives the correct answer. Unreachable for any GRIB2 longitude.
#[test]
...
/// The scanning-mode sign rules must actually be applied: flipping the bits must
/// move the grid, and move it the other way.
#[test]
