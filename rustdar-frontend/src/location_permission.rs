...
mod tests {
    ...
    fn a_remembered_denial_is_never_prompted_again() {
        ...
        let mut f =
            Fixture::new(desktop_with_store(store).with_permission(LocationPermission::Denied));
        for _ in 0..3 {
            f.step_after_the_cadence();
        }
    ...
    fn a_revoked_permission_stops_delivery_and_clears_the_dot() {
        let mut f =
            Fixture::new(TestBridge::desktop().with_permission(LocationPermission::Granted));
        let permission = f.bridge.permission_cell();
        f.step();
