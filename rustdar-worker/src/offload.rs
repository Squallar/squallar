...
pub struct JobRequest {
    ...
    /// A loop frame says [`rustdar_device_profile::constants::LOOP_IMAGE_SIZE`];
...
/// back with [`DescribedOut::take`](rustdar_source::job::DescribedOut::take);
...
impl JobRequest {
    ...
    /// [`EncodeCtx`](rustdar_source::job::EncodeCtx) because one input — the
...
/// `OUT`/`OUT_KIND`/`TAILS` trio the worker posts back (`rustdar_web::worker` is
...
///
/// Implemented by `rustdar-web` over a dedicated `Worker`, and installed rather
...
/// buffer, so `rustdar-web`'s implementation serialises where one that can move
