...
use super::*;
use egui::ColorImage;
use squallar_radar::jobs::{RadarPlanJob, SectionJob, VoxelJob};
...
fn a_real_plan_view_render_lands_on_the_same_picture() {
    ...
    let straight = squallar_radar::render::render_from_sized(&plan.input, side_ceiling_px)
        .expect("the fixture sweep rasterizes")
        .image;
    let side = squallar_device_profile::constants::raster_side_from_rgba_len(straight.len())
        .expect("the rasterizer answers at a side this build makes");

    let frame = execute(&request)
        .and_then(|out| out.take::<squallar_radar::frame::RenderedFrame>())
...
fn a_real_section_cut_lands_on_the_same_picture() {
    ...
    let straight = squallar_radar::xsect::render_section(
...
fn a_voxel_grid_passes_through_the_output_stage_untouched() {
    ...
    let input = &job.input;
    let (scan, declared) = (input.to_scan(), input.declared_nyquist());
    let built = squallar_radar::voxel::build_voxels_with_motion(
        squallar_radar::nyquist::Volume::new(&scan, &declared),
        &job.request,
        input.radar_lat(),
        input.radar_lon(),
