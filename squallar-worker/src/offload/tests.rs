...
use squallar_radar::scan::DecodedScan;
use squallar_radar::voxel::VolumeGrid;
use squallar_radar::voxel::{VoxelRequest, VoxelShape};
use squallar_radar::xsect::CrossSection;
use squallar_radar::xsect::SectionRequest;
use squallar_source::handler::{PaneMut, PaneRef};
use squallar_source::job::{DescribedJob, DescribedOut, JobGeometry};
...
fn a_level3_job() -> JobRequest {
    JobRequest::describe(
        Level3Job {
            ...
            product: squallar_radar::types::RadarProduct::EchoTops,
            radar_lat: 35.0,
            radar_lon: -97.0,
        },
        ceiling_only_geometry(4096),
...
fn a_voxel_request() -> VoxelRequest {
    VoxelRequest {
        centre: (35.0, -97.0),
        half_extent_km: Some(squallar_radar::voxel::HalfExtentKm::square(60.0)),
        base_km_msl: 0.0,
        top_km_msl: 15.0,
        product: squallar_radar::types::RadarProduct::Reflectivity,
...
pub(super) fn an_overlay_alerts_job() -> JobRequest {
    use squallar_overlays::nws::alert::AlertCategory;
...
pub(super) fn an_overlay_outlooks_job() -> JobRequest {
    use squallar_overlays::render::rasterize::OutlooksInput;
    use squallar_overlays::types::{HatchPattern, OverlayFeature};
...
pub(super) fn an_overlay_discussions_job() -> JobRequest {
    use squallar_overlays::render::rasterize::{DiscussionPaint, DiscussionsInput};
    use squallar_overlays::spc::discussion::MdType;
    JobRequest {
        geometry: JobGeometry {
            width: 96,
            height: 64,
...
pub(super) fn an_overlay_reports_job() -> JobRequest {
    use squallar_overlays::render::rasterize::{ReportPaint, ReportsInput};
...
pub(super) fn an_overlay_glm_job() -> JobRequest {
    use squallar_overlays::render::rasterize::{FlashPaint, GlmStrikesInput};
...
fn a_lambert_parts() -> squallar_overlays::hrrr::lambert::LambertGridParts {
    squallar_overlays::hrrr::lambert::LambertGridParts {
...
fn a_model_viewport() -> squallar_geo::GeoBounds {
...
fn a_model_grid() -> squallar_overlays::hrrr::HrrrGridData {
    ...
    let values: Vec<f32> = (0..ni * nj)
        ...
        .collect();
    let geometry = squallar_overlays::hrrr::lambert::LambertGrid::from_parts(a_lambert_parts())
...
pub(super) fn an_overlay_model_whole_job() -> JobRequest {
    use squallar_overlays::render::rasterize::GriddedInput;
    JobRequest {
        geometry: JobGeometry {
            width: 96,
            height: 72,
...
pub(super) fn an_overlay_model_job() -> JobRequest {
    ...
    JobRequest {
        ...
        job: DescribedJob::new(GriddedInput::Window(GridWindow {
            field: squallar_overlays::hrrr::fields::spec(
                squallar_overlays::hrrr::ModelParameter::SurfaceBasedCape,
            )
            .id
            .clone(),
            ni: 60,
            nj: 44,
            coords: squallar_overlays::hrrr::GridCoords::Lambert(lambert),
            win: IndexWindow {
                i0: 14,
                i1: 20,
                j0: 10,
...
fn a_rectangular_voxel_job() -> JobRequest {
    JobRequest::describe(
        VoxelJob {
            input: Box::new(a_volume_input()),
            request: VoxelRequest {
                half_extent_km: Some(squallar_radar::voxel::HalfExtentKm {
                    east_km: 92.0,
                    north_km: 37.0,
                }),
                ..a_voxel_request()
...
fn a_request_naming_a_different_product_from_its_payload_is_refused() {
    ...
    for (job, product_offset) in [(a_section_job(), 45), (a_voxel_job(), 46)] {
        ...
        let code = squallar_radar::types::RadarProduct::Velocity.wire_code();
...
fn a_reply_payload_of_the_wrong_kind_is_refused_by_the_rows_codec() {
    ...
    let grid_bytes =
        squallar_radar::voxel::to_bytes(&grid).expect("a registered field has a wire code");
...
fn a_decoded_volume_comes_back_under_its_own_out_kind() {
    ...
    // has its own round-trip row in `squallar-radar`.
    ...
    let volume = squallar_radar::scan::DecodedScan {
        scan: nexrad_model::data::Scan::new(pattern, Vec::new()),
        declared_nyquist: squallar_radar::nyquist::DeclaredNyquist::empty(),
    };
...
fn overlay_reply_via_wire(
    ...
    Vec<u8>,
    Option<squallar_overlays::render::rasterize::HitCells>,
) {
    let RasterizeOutput {
        rgba, hit_cells, ..
...
fn the_alerts_render_is_byte_identical_direct_and_via_the_wire() {
    ...
    let (width, height, bounds) = (geometry.width, geometry.height, geometry.bounds);
    let alerts = job
        .downcast_ref::<squallar_overlays::render::rasterize::AlertsInput>()
        .expect("the fixture is an alerts job");

    let direct =
        squallar_overlays::render::rasterize::rasterize_nws_alerts(alerts, &bounds, width, height);
    assert_eq!(
        direct.alpha,
        squallar_overlays::render::rasterize::AlphaMode::Premultiplied,
    ...
    let unhidden = squallar_overlays::render::rasterize::AlertsInput {
...
fn the_outlooks_render_is_byte_identical_direct_and_via_the_wire() {
    ...
    let (width, height, bounds) = (geometry.width, geometry.height, geometry.bounds);
    let outlooks = job
        .downcast_ref::<squallar_overlays::render::rasterize::OutlooksInput>()
        .expect("the fixture is an outlooks job");

    let direct = squallar_overlays::render::rasterize::rasterize_spc_outlooks(
...
fn the_discussions_render_is_byte_identical_direct_and_via_the_wire() {
    ...
    let (width, height, bounds) = (geometry.width, geometry.height, geometry.bounds);
    let discussions = job
        .downcast_ref::<squallar_overlays::render::rasterize::DiscussionsInput>()
        .expect("the fixture is a discussions job");

    let direct = squallar_overlays::render::rasterize::rasterize_spc_discussions(
        discussions,
        &bounds,
        width,
        height,
...
fn ids_of(cells: &squallar_overlays::render::rasterize::HitCells) -> HashSet<u32> {
    cells.cells.values().flatten().copied().collect()
}

/// The UV at the centre of cell `idx`, in the coordinates `HitMap::hit_test`
/// takes.
fn uv_of_cell(cells: &squallar_overlays::render::rasterize::HitCells, idx: u32) -> (f32, f32) {
...
fn the_reports_render_is_byte_identical_direct_and_via_the_wire() {
    ...
    let (width, height, bounds) = (geometry.width, geometry.height, geometry.bounds);
    let reports = job
        .downcast_ref::<squallar_overlays::render::rasterize::ReportsInput>()
        .expect("the fixture is a reports job");

    let direct = squallar_overlays::render::rasterize::rasterize_storm_reports(
...
fn the_glm_render_is_byte_identical_direct_and_via_the_wire() {
    ...
    let (width, height, bounds) = (geometry.width, geometry.height, geometry.bounds);
    let glm = job
        .downcast_ref::<squallar_overlays::render::rasterize::GlmStrikesInput>()
        .expect("the fixture is a GLM job");

    let direct =
        squallar_overlays::render::rasterize::rasterize_glm_strikes(glm, &bounds, width, height);
    assert_eq!(
        direct.alpha,
        squallar_overlays::render::rasterize::AlphaMode::Premultiplied,
...
fn a_worker_that_re_read_its_own_clock_would_fail_the_glm_parity() {
    ...
    let rederived = squallar_overlays::render::rasterize::GlmStrikesInput {
        now: glm.now + chrono::Duration::seconds(60),
        ..glm.clone()
    };
    let drifted = squallar_overlays::render::rasterize::rasterize_glm_strikes(
...
/// answers.
fn a_seeded_reports_registry() -> squallar_overlays::render::overlay_state::OverlayRegistry {
    ...
    registry.apply_fetch_result(
        OverlayFetchResult {
            kind: squallar_source::id::known::STORM_REPORTS,
...
fn a_seeded_glm_registry() -> squallar_overlays::render::overlay_state::OverlayRegistry {
    ...
    registry.apply_fetch_result(
        OverlayFetchResult {
            kind: squallar_source::id::known::LIGHTNING,
...
fn a_zip_ctx() -> squallar_overlays::render::overlay_state::RasterizeContext {
...
fn the_hit_map_zip_answers_the_direct_calls_hits_on_a_probe_grid() {
    ...
    for (registry, kind) in [
        ...
        let direct = if let Some(input) =
            job.downcast_ref::<squallar_overlays::render::rasterize::ReportsInput>()
        {
            assert_eq!(items.len(), input.reports.len(), "one item per row");
            squallar_overlays::render::rasterize::rasterize_storm_reports(
            ...
            job.downcast_ref::<squallar_overlays::render::rasterize::GlmStrikesInput>()
        {
            ...
            squallar_overlays::render::rasterize::rasterize_glm_strikes(
                input, &bounds, width, height,
            )
        } else {
...
fn a_hit_cells_fixture() -> squallar_overlays::render::rasterize::HitCells {
    ...
    squallar_overlays::render::rasterize::HitCells {
        width: 4,
        height: 2,
        cells,
    }
...
fn the_frame_reply_framing_is_the_one_this_registry_ships() {
    ...
    let full = RenderedFrame {
        ...
        melting_layer_source: Some(squallar_radar::hca::MeltingLayerSource::RadarDetected),
        storm_motion: Some(squallar_radar::srv::SrvMotion {
            speed_kt: 33.5,
            direction_deg: 245.0,
    ...
    match JobRequest::from_bytes(&rekinded) {
        Some(JobRequest { job, .. }) => {
            ...
            assert_eq!(
                ...
                squallar_overlays::spc::reports::StormReportKind::Wind,
...
fn the_model_render_is_byte_identical_direct_and_via_the_wire() {
    ...
    let direct =
        squallar_overlays::render::rasterize::rasterize_gridded(model, &bounds, width, height);
    assert_eq!(
        direct.alpha,
        squallar_overlays::render::rasterize::AlphaMode::Straight,
...
fn the_whole_model_grid_encodes_as_exactly_its_window() {
    use squallar_overlays::render::rasterize::{GridWindow, GriddedInput};
    ...
    assert_eq!(
        *model,
        GriddedInput::Window(GridWindow {
            field: squallar_overlays::hrrr::fields::spec(grid.parameter)
                .id
                .clone(),
            ni: grid.ni,
            nj: grid.nj,
...
fn a_malformed_model_job_is_refused_rather_than_misread() {
    ...
    let decoded_model = |bytes: &[u8]| match JobRequest::from_bytes(bytes) {
        Some(JobRequest { job, .. }) => job
            .downcast_ref::<squallar_overlays::render::rasterize::GriddedInput>()
            .cloned(),
        _ => None,
    };
    ...
    match model.coords() {
        squallar_overlays::hrrr::GridCoords::Lambert(grid) => assert_eq!(
            grid.to_parts().dx,
            1500.0,
...
fn a_malformed_outlooks_job_is_refused_rather_than_misread() {
    use squallar_overlays::types::HatchPattern;
