...
`PaneState` holds per-pane: `selected_product`, `selected_elevation`, `radar_image: Option<RadarImageData>`, `layers: LayerManager`, `map_memory: MapMemory` (walkers viewport), overlay texture caches (`spc_overlay_texture`, `nws_alert_texture`, `spc_md_texture` — all `OverlayTextureCache`), and `loop_state: Option<LoopPlaybackState>` (radar loop). `PaneLayout` defines grid arrangements: 1–6 panes desktop (`MAX_PANES_DESKTOP = 6`), 1–4 mobile (`MAX_PANES_MOBILE = 4`). Predefined layouts: 1→[1], 2→[2], 3→[2,1], 4→[2,2], 5→[3,2], 6→[3,3].
