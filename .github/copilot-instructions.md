...
| `rustdar-overlays` | Weather overlay data and render-agnostic overlay logic. Data: SPC convective outlooks (Days 1–8, GeoJSON), Mesoscale Discussions (RSS via `roxmltree` + `LAT...LON` polygon extraction), and NWS alerts (API + zone geometry resolution with disk caching). Pre-computes triangulations (`earcutr`) and geo-bounds at fetch time. Simplifies polygons with Ramer-Douglas-Peucker (ε=`SIMPLIFY_EPSILON` 0.005° in `types.rs`). METAR/ASOS surface observations fetched from Aviation Weather Center bulk cache (`aviationweather.gov/data/cache/metars.cache.csv.gz`), parsed in `metar/` module (`MetarOb`, `FlightCategory`, `CloudLayer`). Types: `OverlayFeature`, `GeoPolygon`, `HatchPattern`, `ScreenPoint`, `OverlayLabel`, `SpcOutlook`, `SpcDiscussion`, `NwsAlert`, `MetarOb`. `OverlayLabel { lat, lon, text, color }` is a generic map label drawn by the UI. Alert enums (`AlertSeverity`, `AlertUrgency`, `AlertCertainty`) use a `str_enum!` macro for boilerplate-free `FromStr` impls. Centralized alpha constants (`CIG_FILL_ALPHA`, `REGULAR_FILL_ALPHA`, `NWS_FILL_ALPHA`, `STROKE_ALPHA`) in `types.rs`. The `render` submodule contains: `rasterize.rs` (tiny-skia texture rasterization — see Overlay Rendering below; includes `rasterize_radar_sites()` for NEXRAD site markers), `geo.rs` (screen-space geometry via `ScreenPoint`: `point_in_polygon`, `aabb`, `clip_line_to_polygon`, plus polygon simplification and triangulation), `hatch.rs` (CIG hatch line generation), `layers.rs` (`LayerKind` enum + `LayerManager`), `draw.rs` (GUI-agnostic `PointPainter` trait + `TextAnchor`, `MapPoint`, `DrawPointContext`, `HoverContext` — see Per-Frame Point Rendering below), `station_model.rs` (NWS station model plots: wind barbs, cloud cover circles, temperature/dewpoint/altimeter text layout — uses `PointPainter` for all drawing), `overlay_state.rs` (`OverlayState<T>`, `OverlayRegistry`, `OverlayHandler` trait, `OverlayKind` enum with 9 variants — 6 texture-overlay + 1 per-frame point overlay + 2 non-texture map layers, `ClickableItem<'a>` struct, `SelectedOverlay` enum, `FetchConfig`, `RasterizeContext`, `FetchTask` — unified map layer identifier used across all crates for render dispatch, texture caching, draw ordering, click detection, and channel responses). `OverlayKind` has `is_texture_overlay()`/`texture_overlays()`/`default_draw_order()` helpers. `OverlayHandler` trait has optional per-frame point rendering methods: `per_frame_points()`, `draw_point()`, `point_hit_radius()`, `hover_text()`. `OverlayRegistry` delegates to per-type `OverlayHandler` impls for data access, fetch task creation, rasterization prep, popup content, clickable items, and per-frame point rendering — the UI crate is overlay-type-agnostic. `handlers/` submodule contains `SpcOutlookHandler`, `SpcDiscussionHandler`, `NwsAlertHandler`, `StormReportsHandler`, `MetarHandler`. `SpcDiscussion` has a `feature: OverlayFeature` field (constructed at parse time) for uniform click detection. |
...
- **Hit buffer system:** Point-marker overlays (storm reports) produce a `HitMap` alongside RGBA pixels during rasterization. `HitMap` is quarter-resolution (1/4 each axis), stored as a sparse `HashMap<u32, Vec<u32>>` (cell index → item IDs) + `HashMap<u32, SelectedOverlay>` (item ID → selection). During click detection, screen position is projected to texture UV coordinates via `overlay_texture_rect()`, then `HitMap::hit_test(u, v)` returns all overlapping items at that pixel. This makes click detection pixel-perfect — the exact rendered circle pixels are clickable, with no duplicated radius formulas or zoom-dependent hacks.
