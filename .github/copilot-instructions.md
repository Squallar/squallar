...
| `rustdar-egui` | Pure egui UI layer. Defines `Gui` (map + controls) and `GuiAction` enum. Uses the `walkers` crate for map tiles (CartoDB no-labels + labels-only overlay). No wgpu dependency. Large `Gui` impl is split across seven `#[path]` submodules: `ui_popups.rs`, `ui_config.rs`, `ui_mobile.rs`, `ui_map_overlays.rs`, `ui_desktop.rs`, `ui_map.rs`, `ui_settings.rs`. `ui_map.rs` has its own `#[path]` submodule `ui_map_pane.rs` for per-pane map content rendering (`PaneRenderCtx`, `render_pane_map_content()`). Public modules: `actions.rs` (GuiAction/RadarConfig), `overlay_cache.rs` (texture-based overlay cache — see Overlay Rendering below), `pane.rs` (PaneState/PaneLayout/RadarImageData), `tiles.rs` (CartoDB tile source + slippy-map math). Internal modules: `point_painter.rs` (`EguiPointPainter` struct implementing `PointPainter` trait — bridges overlay-crate drawing to egui `Painter`), `geo.rs` (provides `dashed_line_shapes()` and `Pos2` ↔ `ScreenPoint` conversion helpers: `to_screen`, `to_pos2`, `slice_to_screen`). |
| `rustdar-units` | Leaf crate for user-facing unit conversion and timezone formatting. Defines `UserPreferences` struct (persisted in `ui.json`) with per-measurement unit enums: `SpeedUnit` (mph, m/s, km/h, kt), `DistanceUnit` (km, mi, nmi), `HeightUnit` (ft, m), `PrecipRateUnit` (in/hr, mm/hr), `HailSizeUnit` (in, cm, mm), `TimezonePreference` (Local, UTC). All enums implement `Serialize`/`Deserialize` with `#[serde(default)]` for backward-compatible config. Conversion functions convert from internal units (m/s, km, kft, in/hr, kt, inches) at display boundaries only — internal data stays in original units. `TimezonePreference` provides `format_naive_utc()`, `format_rfc3339()`, and `utc_to_display()` helpers. No GUI dependency. |
| `rustdar-radar` | Radar data logic: fetching Level II scans from AWS and Level III products from TGFTP (`scan.rs`), rendering to 2048×2048 RGBA images via Web Mercator projection (`render.rs`), data-driven color palettes via `ColorScale` lookup tables (`palette.rs`), the static site database (`sites.rs`, 207 NEXRAD sites via `LazyLock<HashMap>`), and shared data types (`types.rs`: `RadarProduct` with 13 variants, `ScanInfo`, `ImageBounds`, constants). Uses `rayon` for parallel rendering with `AtomicU32` buffers. `render.rs` also contains product-specific value conversion helpers moved from `nexrad-level3`: `build_vil_lut()`, `decode_legacy_thresholds()`, `nexrad_float16()`. `RadarProduct::format_value()` accepts `&UserPreferences` for unit conversion at display time. |
| `rustdar-overlays` | Weather overlay data and render-agnostic overlay logic. Data: SPC convective outlooks (Days 1–8, GeoJSON), Mesoscale Discussions (RSS via `roxmltree` + `LAT...LON` polygon extraction), NWS alerts (API + zone geometry resolution with disk caching), and HRRR model data (CIN via NOMADS filter, `grib` crate for GRIB2 parsing — see `hrrr/` module). Pre-computes triangulations (`earcutr`) and geo-bounds at fetch time. Simplifies polygons with Ramer-Douglas-Peucker (ε=`SIMPLIFY_EPSILON` 0.005° in `types.rs`). METAR/ASOS surface observations fetched from Aviation Weather Center bulk cache (`aviationweather.gov/data/cache/metars.cache.csv.gz`), parsed in `metar/` module (`MetarOb`, `FlightCategory`, `CloudLayer`). Types: `OverlayFeature`, `GeoPolygon`, `HatchPattern`, `ScreenPoint`, `OverlayLabel`, `SpcOutlook`, `SpcDiscussion`, `NwsAlert`, `MetarOb`. `OverlayLabel { lat, lon, text, color }` is a generic map label drawn by the UI. Alert enums (`AlertSeverity`, `AlertUrgency`, `AlertCertainty`) use a `str_enum!` macro for boilerplate-free `FromStr` impls. Centralized alpha constants (`CIG_FILL_ALPHA`, `REGULAR_FILL_ALPHA`, `NWS_FILL_ALPHA`, `STROKE_ALPHA`) in `types.rs`. The `render` submodule contains: `rasterize.rs` (tiny-skia texture rasterization — see Overlay Rendering below; includes `rasterize_radar_sites()` for NEXRAD site markers), `geo.rs` (screen-space geometry via `ScreenPoint`: `point_in_polygon`, `aabb`, `clip_line_to_polygon`, plus polygon simplification and triangulation), `hatch.rs` (CIG hatch line generation), `layers.rs` (`LayerKind` enum + `LayerManager` — legacy, being phased out in favor of handler-owned state), `controls.rs` (declarative UI control descriptors: `ControlItem` enum with Toggle/Section/Dropdown/Slider/ButtonRow/InfoText/Separator variants, `ControlUpdate`, `ControlValue`, `PaneControlContext`), `draw.rs` (GUI-agnostic `PointPainter` trait + `TextAnchor`, `MapPoint`, `DrawPointContext`, `HoverContext` — see Per-Frame Point Rendering below), `station_model.rs` (NWS station model plots: wind barbs, cloud cover circles, temperature/dewpoint/altimeter text layout — uses `PointPainter` for all drawing), `overlay_state.rs` (`OverlayState<T>`, `OverlayRegistry`, `OverlayHandler` pub trait, `OverlayItem` trait for type-erased selected items, `RenderMode` enum (Texture/PerFramePoint/PerFrameDirect/Tile), `OverlayKind` enum with 12 variants, `ClickableItem` struct, `FetchConfig`, `RasterizeContext`, `FetchTask`, `PopupAction` with `Arc<dyn OverlayItem>` target — unified map layer identifier used across all crates for render dispatch, texture caching, draw ordering, click detection, and channel responses). `OverlayKind` has `all()` and `default_draw_order()` helpers. `OverlayHandler` trait has ~25 methods: identity (`kind`, `display_name`, `render_mode`, `default_enabled`, `is_enabled`), data lifecycle (`data_generation`, `has_data`, `is_fetching`, `set_fetching`, `fetch_time`, `auto_poll_interval`, `item_count`), rendering (`prepare_rasterize`, `create_fetch_tasks`, `apply_fetch_result`, `retain_selections`, `clickable_items`, `handle_popup_action`), per-frame point methods (`per_frame_points`, `draw_point`, `point_hit_radius`, `hover_text`). Handlers own their toggle/config state internally (e.g., `NwsAlertHandler.enabled_categories`, `SpcOutlookHandler.selected_day`/`enabled_products`). Each handler defines an `XxxItem` struct implementing `OverlayItem` for type-erased selection/popup identity. `OverlayRegistry` delegates to per-type `OverlayHandler` impls — the UI crate is overlay-type-agnostic. `handlers/` submodule contains `SpcOutlookHandler`, `SpcDiscussionHandler`, `NwsAlertHandler`, `StormReportsHandler`, `MetarHandler`, `ModelDataHandler`. `SpcDiscussion` has a `feature: OverlayFeature` field (constructed at parse time) for uniform click detection. |
...
**Data flow:** `Gui::ui()` returns `Vec<GuiAction>` → `App::process_gui_actions()` dispatches via `handle_radar_action()` / `handle_overlay_action()` in `app_fetch.rs` → fetches spawn on `App::tokio_runtime` → results arrive via `std::sync::mpsc` channels in `ChannelHub` → radar rendering runs on `std::thread::spawn` (not Tokio) with rayon parallelism → `apply_render_to_pane()` stores the rendered texture + `RadarTextureMeta` (value_data, lat, lon, max_range_km) in the per-pane `OverlayTextureCache` for `OverlayKind::Radar` — radar images are now drawn via the same `draw_overlay_texture()` pipeline as all other texture overlays. Overlay fetching uses trait-based dispatch: `fetch_overlay()` calls `OverlayRegistry::create_fetch_tasks()` which delegates to the handler's `create_fetch_tasks()` → spawns async tasks → results arrive as `OverlayFetchResult { kind, data: Box<dyn Any + Send> }` → `apply_fetch_result()` routes to the correct handler. Overlay rendering follows a parallel path: `GuiAction::RenderOverlay` → `spawn_overlay_render()` calls `OverlayRegistry::prepare_rasterize()` for handler-backed overlays → background `std::thread::spawn` with tiny-skia rasterization → `OverlayRenderResponse` via `overlay_render` channel → `poll_overlay_render_results()` uploads texture → `draw_overlay_texture()` per frame.

## Key Types & Data Structures

### ChannelHub (`rustdar-platform/src/channels.rs`)
Eight channel pairs for async communication: `scan` (Level II fetch results with `ScanResponse`), `render` (completed radar images with `RenderResponse`), `level3` (Level III product fetches), `overlay_fetch` (unified overlay fetch results via `OverlayFetchResult`), `overlay_render` (background-rasterized overlay textures with `OverlayRenderResponse`), `loop_scan_list` (scan listing results for loop initialization), `loop_scan_download` (individual scan downloads for loop frames), `loop_render` (completed loop frame renders). All use `std::sync::mpsc` with non-blocking `try_recv()` polling in the event loop.
...
`PaneState` holds per-pane: `selected_product`, `selected_elevation`, `loading_site: Option<String>` (site currently being fetched for this pane), `radar_sites_render_gen: u64` (bumped to trigger RadarSites texture re-render), `layers: LayerManager`, `map_memory: MapMemory` (walkers viewport), `overlay_textures: HashMap<OverlayKind, OverlayTextureCache>` (keyed by `OverlayKind`, texture-overlay kinds including Radar have entries, accessed via `overlay_cache()`/`overlay_cache_mut()` helpers), `draw_order: Vec<OverlayKind>` (visual stacking order, bottom-to-top, persisted in `ui.json`), and `loop_state: LoopPlaybackState` (radar loop). `PaneLayout` defines grid arrangements: 1–6 panes desktop (`MAX_PANES_DESKTOP = 6`), 1–4 mobile (`MAX_PANES_MOBILE = 4`). Predefined layouts: 1→[1], 2→[2], 3→[2,1], 4→[2,2], 5→[3,2], 6→[3,3].

### Radar Loop System (`rustdar-egui/src/pane.rs`, `rustdar-platform/src/app_fetch.rs`, `rustdar-platform/src/app_render.rs`)
Per-pane radar animation loops. `LoopPlaybackState` holds `playing`, `current_frame`, `frames: Vec<LoopFrame>`, `lookback_secs`, `fetching`, `last_advance`, `site_lat`, `site_lon`. The `site_lat/lon` fields are captured from `scan_info` when the loop is created and used for all loop rendering — decoupled from the global `scan_info` so loops work correctly when the user navigates to historical times. Each `LoopFrame` has a `timestamp`, optional `texture: Option<RadarImageData>`, and `render_in_flight` flag. Loop pipeline: `GuiAction::EnableLoop` → `handle_enable_loop()` lists historical scans via `list_scans_for_range()` on AWS S3 → `poll_loop_scan_list_results()` populates frames and starts downloads → `poll_loop_scan_download_results()` caches downloaded scans in `App::loop_scan_cache` → `dispatch_loop_renders()` renders frames around playhead (budget ~30) → `poll_loop_render_results()` uploads textures. `advance_loop_playback()` skips to the next textured frame based on `loop_speed_fps` timing. Growing loops: `append_scan_to_active_loops()` adds new auto-polled scans and evicts frames past the lookback window. Loop end time uses the scan's timestamp (not `Utc::now()`), so historical scans produce correctly-anchored loops. `Gui` holds `loop_lookback_secs` (default 3600) and `loop_speed_fps` (default 5.0), both persisted in `ui.json`.

### LayerKind (`rustdar-overlays/src/render/layers.rs`)
Layer toggles: `Radar`, `SpcCategorical`, `SpcTornado`, `SpcWind`, `SpcHail`, `SpcProbabilistic`, `SpcMesoscaleDiscussions`, `NwsWarnings`, `NwsWatches`, `NwsAdvisories`, `StormReports`, `Metar`, `CityLabels`, `RadarSites`. Defaults: Radar/MDs/NWS alerts/CityLabels enabled; SPC outlooks/StormReports/Metar/RadarSites disabled.

### Overlay Texture Cache (`rustdar-egui/src/overlay_cache.rs`)
Texture-based overlay rendering. `OverlayTextureCache` (per overlay type per pane) holds an `Option<OverlayTextureData>` with the current rendered texture, `render_in_flight` flag, and `render_generation` counter. `OverlayTextureData` stores the egui `TextureHandle`, `geo_bounds`, `data_generation`, `render_zoom` (quantized as `zoom * 32`), pixel dimensions, optional `radar_meta: Option<RadarTextureMeta>`, and optional `hit_map: Option<HitMap>` (for pixel-perfect click detection on point overlays). `RadarTextureMeta` bundles `value_data: Arc<Vec<f32>>`, `lat`, `lon`, `max_range_km` for radar-specific hover/tooltip lookups. `needs_rerender()` triggers on data generation change, zoom level change, or pan exceeding the overdraw margin (`OVERDRAW_FRACTION = 1.0`, `PAN_REBUILD_THRESHOLD = 0.7`). `draw_overlay_texture()` projects the texture's NW/SE corners to screen space and emits a single `painter.image()` call — near-zero per-frame cost. `overlay_texture_rect()` computes the screen rect for a texture (used by both drawing and hit-testing). `geo_point_in_feature()` provides click hit-testing using Web Mercator Y projection. `viewport_geo_bounds()` extracts the current map viewport's geographic extent.

### Time Navigation & Live/Historic Mode (`rustdar-egui/src/ui.rs`, `rustdar-platform/src/app_fetch.rs`)
`Gui` tracks `viewing_live: bool` (default `true`) — whether the displayed scan is the most recent. Time navigation UI provides forward/back buttons and a step size dropdown (1 scan, 10 min, 30 min, 1 hr, 2 hr, 6 hr, 12 hr) in the layers panel. A "Live" button returns to real-time mode. `GuiAction::NavigateTime { step_secs }` moves by relative offset (positive=forward, capped to now). `GuiAction::NavigateOneScan { forward }` navigates to the adjacent scan on AWS via `get_adjacent_scan()`. `GuiAction::JumpToLive` restores live viewing, applying any cached auto-poll scan or fetching latest. Auto-poll radar checks in `check_auto_polls()` only fire when `viewing_live`; when historic, auto-poll results are cached in `App::latest_cached_scan` for instant live return and loop growing. NWS/SPC overlay auto-polls always run. Time navigation with an active loop triggers `reinit_active_loops()` to rebase the loop around the new scan time. `time_step_secs` (default 600) is persisted in `ui.json`.
...
- **`#[path]` submodule pattern:** Large files like `ui.rs` and `app.rs` are split using `#[path = "ui_xxx.rs"] mod xxx;` to avoid converting to directory modules. `ui.rs` has seven submodules: `popups`, `config`, `mobile`, `map_overlays`, `desktop`, `map`, `settings`. `ui_map.rs` has a nested submodule `pane_render` via `ui_map_pane.rs`. `app.rs` has two: `fetch` and `render`. Extracted methods use `impl super::Gui {}` (or `impl super::App {}`) blocks with `pub(super)` visibility. Free functions use `pub(super) fn`.
...
- **Config persistence:** `Gui` saves/loads `ui.json` (JSON format via serde_json; fields: pane_count, viewport_sync, sync_layers, auto_poll, site, preferences) from `Gui::default_config_dir()` (`XDG_CONFIG_HOME/rustdar` or `~/.config/rustdar`). `preferences` is a `UserPreferences` struct with `#[serde(default)]` for backward-compatible config evolution. Loaded at `App::new()`, saved on exit.
...
- **UI ↔ Platform boundary:** `rustdar-egui` must not depend on wgpu/winit. It communicates intent through `GuiAction` and receives state via setter methods on `Gui` (e.g., `set_scan_info()`). Radar renders are stored directly into the per-pane `OverlayTextureCache` for `OverlayKind::Radar` by the platform crate via `pane.overlay_cache_mut()`. `Gui::ui(&mut self, &egui::Context) -> Vec<GuiAction>` is the main entry point called each frame. `Gui` exposes `active_pane()` / `active_pane_mut()` for direct access to the current pane's `PaneState`. Android-only fields (`show_menu`, `double_tap_detector`) are grouped into a `MobileState` substruct gated with `#[cfg(target_os = "android")]`.
...
- **Radar image data:** For the static (non-loop) radar render, the RGBA texture is stored in the `OverlayTextureCache` for `OverlayKind::Radar` and metadata (value_data, lat, lon, max_range_km) lives in `RadarTextureMeta` on the cache entry. `RadarImageData` struct (in `pane.rs`) is retained only for loop playback frames in `LoopFrame.texture`. Range ring and hover/tooltip rendering are done per-frame from the cached metadata.
- **ImageBounds** (in `rustdar-radar/src/types.rs`) computes geographic bounds from a radar site lat/lon for the 2048×2048 image. Uses Web Mercator Y for vertical alignment with map tiles. `geo_to_pixel()` converts lat/lon → pixel coordinates for hover value lookup.
...
- **Overlay rendering (texture-based):** Overlays (SPC outlooks, MDs, NWS alerts, storm reports, radar sites) are rasterized to RGBA textures on background threads using **tiny-skia**, then displayed as geo-positioned images via `painter.image()` — the same approach as the radar image itself. Radar images are rendered by `rustdar-radar` (rayon parallelism) and stored into the overlay cache with `RadarTextureMeta` for hover/tooltip support. The pipeline: `render_pane_map_content()` checks `needs_rerender()` on each `OverlayTextureCache` (skipping `Radar`, which is driven by `dispatch_pane_renders()`) → emits `GuiAction::RenderOverlay` with viewport bounds, dimensions, zoom → `app_fetch.rs::spawn_overlay_render()` expands bounds by `OVERDRAW_FRACTION` (clamped to ±85.05° lat for valid Mercator), calls `OverlayRegistry::prepare_rasterize()` for handler-backed overlays (which returns a closure capturing cloned data), spawns `std::thread::spawn` → rasterize functions produce `RasterizeOutput` (RGBA pixels + optional `HitMap`) → `OverlayRenderResponse` sent via channel → `poll_overlay_render_results()` uploads as egui texture and stores `HitMap` → `draw_overlay_texture()` projects corners and draws each frame. Per-frame cost is one `painter.image()` per overlay type. `OverlayDrawContext` in `ui_map_overlays.rs` provides a single generic `draw_overlay()` method that draws the texture, renders labels, and performs click hit-testing — when a `HitMap` is present (point overlays like storm reports) it does pixel-perfect UV lookup, otherwise falls back to `geo_point_in_feature()` for polygon containment. CIG hatching uses a two-pass system in `rasterize_spc_outlooks()`: pass 1 fills/strokes polygons, pass 2 generates hatch lines with exclusion masks (lower-CIG hatches masked by higher-CIG fill zones via tiny-skia `Mask`). CIG1 = dotted 45° lines, CIG2 = solid 135° lines, CIG3 = cross-hatch. Stroke widths scale down with polygon screen size via `scaled_stroke_width()` (clamped to [0.5, base]).
- **Hit buffer system:** Point-marker overlays (storm reports) produce a `HitMap` alongside RGBA pixels during rasterization. `HitMap` is quarter-resolution (1/4 each axis), stored as a sparse `HashMap<u32, Vec<u32>>` (cell index → item IDs) + `HashMap<u32, Arc<dyn OverlayItem>>` (item ID → selection). During click detection, screen position is projected to texture UV coordinates via `overlay_texture_rect()`, then `HitMap::hit_test(u, v)` returns all overlapping items at that pixel. This makes click detection pixel-perfect — the exact rendered circle pixels are clickable, with no duplicated radius formulas or zoom-dependent hacks.
- **Per-pane map rendering:** `ui_map_pane.rs` (submodule of `ui_map.rs`) contains `PaneRenderCtx` struct and `render_pane_map_content()` entry point. Uses a draw-order dispatch loop: iterates `pane.draw_order` and matches each `OverlayKind` variant to the appropriate draw call. Texture overlays (SPC/NWS/RadarSites) draw via `OverlayDrawContext::draw_overlay()`. Radar draws its overlay texture + per-frame range ring via `render_radar_range_ring()` + hover via `update_pane_hover_value_from_meta()` (using `RadarTextureMeta` from the cache). When a loop is active (`multi_frame`), radar falls back to `render_radar_overlay()` using `RadarImageData` from loop frames. RadarSites also renders per-frame text labels and click handling via `handle_radar_site_interactions()` (tiny-skia can't render text). CityLabels and UserLocation draw directly per-frame. Render-trigger loop iterates `OverlayKind::texture_overlays()` (skipping Radar, which is driven by `dispatch_pane_renders()`), checking `needs_rerender()` on each `OverlayTextureCache` and pushing `GuiAction::RenderOverlay` when needed.
- **Viewport sync:** When enabled, snapshots zoom+position before rendering, detects which pane changed, and propagates to all others. Layer sync copies active pane's layer states and `draw_order` to all panes.
- **Auto-polling:** Radar scans poll every 60s (exponential backoff on error, capped at 300s, reset to 60s on success) but only when `viewing_live`. When viewing historic data, auto-poll results are silently cached in `App::latest_cached_scan` for instant restore on `JumpToLive` and for loop growing via `append_scan_to_active_loops()`. NWS alerts and SPC MDs auto-refresh every 120s when their layers are enabled (regardless of live/historic mode).
- **Radar loop pipeline:** Per-pane animation loops fetch historical Level II scans from AWS S3, render frames to 2048×2048 textures on background threads, and animate by swapping textures on a timer. Loop frames still use `RadarImageData` stored in `LoopFrame.texture` (not the overlay cache, since multiple frames cycle). Loop lifecycle: `EnableLoop` → list scans for time range → download each scan → render frames around playhead (budget ~30) → animate via `advance_loop_playback()`. When auto-poll delivers a new scan, `append_scan_to_active_loops()` appends to active loops and evicts old frames past the lookback window. Loop frames are independent of (and override) the static radar image when present. UI controls: enable/disable checkbox, lookback slider (5 min–24 hr, logarithmic), speed slider (1–30 fps), play/pause/step/seek transport.
...
- `ModelParameter` enum: `SurfaceBasedCin` (var_CIN, lev_surface), `MixedLayerCin` (var_CIN, lev_180-0_mb_above_ground). Extensible to CAPE/SRH/shear.
...
- Color scale: transparent 0→−25, green→yellow −25→−50, yellow→orange −50→−100, orange→red −100→−200, red→purple −200→−500+ J/kg.
- `ModelDataHandler` owns toggle + parameter dropdown (SBCIN/MLCIN), auto-polls every 3600s. `hover_value_at()` does nearest-neighbor lookup on the grid.
...
|---|---|---|
| `IMAGE_SIZE` | 2048 | `rustdar-radar/src/types.rs` |
| `MAX_RANGE_KM` | 230.0 | `rustdar-radar/src/types.rs` |
| `PIXELS_PER_KM` | ~4.45 | Derived: 2048 / (2 × 230) |
...
| `OVERDRAW_FRACTION` | 1.0 | `rustdar-egui/src/overlay_cache.rs` |
...
4. Add `OverlayKind` variant in `rustdar-overlays/src/render/overlay_state.rs` with `is_enabled()` arm. If it's a texture overlay, add to `texture_overlays()`. Add to `all()` and `default_draw_order()` at the appropriate position.
...
6. Create a new handler in `rustdar-overlays/src/render/handlers/` implementing `OverlayHandler` trait (follow `SpcOutlookHandler` or `NwsAlertHandler` patterns). The handler encapsulates: data state (`OverlayState<T>`), fetch logic (`create_fetch_tasks`), render logic (`prepare_rasterize`), popup content, and clickable items.
7. Register the handler in `rustdar-overlays/src/render/handlers/mod.rs` `create_handlers()`. If non-texture, add a draw branch in the `ui_map_pane.rs` draw-order dispatch loop.
