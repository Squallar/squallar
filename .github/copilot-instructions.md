...
Five crates in a Cargo workspace (`resolver = "2"`, edition 2024):

| Crate | Role |
|---|---|
| `rustdar-platform` | Main binary + lib. Owns the winit event loop, wgpu surface, egui renderer, and Tokio runtime. Entry points: `src/main.rs` (desktop), consumed by `rustdar-android` on Android. |
| `rustdar-egui` | Pure egui UI layer. Defines `Gui` (map + controls) and `GuiAction` enum. Uses the `walkers` crate for map tiles (CartoDB). No wgpu dependency. |
| `rustdar-radar` | Radar data logic: fetching scans from AWS (`scan.rs`), rendering to 1800×1800 RGBA images (`render.rs`), color palettes (`palette.rs`), and the static site database (`sites.rs`, 207 NEXRAD sites). Uses `rayon` for parallel rendering. |
...
**Data flow:** `Gui` emits `GuiAction` variants → `App::handle_gui_action()` dispatches fetches on a shared `tokio::runtime::Runtime` → results arrive via `std::sync::mpsc` channels → radar rendering runs on `std::thread::spawn` (not Tokio) → image textures are loaded into egui.
...
1. Add variant to `RadarProduct` enum in `rustdar-radar/src/render.rs`
2. Implement `code()`, `name()`, `get_moment()` arms, and add to `all()`
3. Add color function in `palette.rs` and wire it in `get_color_for_value()`
4. The UI picks up new products automatically from `ScanInfo::available_products`
